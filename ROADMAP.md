...
- [ ] **Task 45: Include derived analytics in `mix ccxt_extract.update`** [D:3/B:5/U:4 → Eff:1.50] 📋 — `mix ccxt_extract.update` runs QuickBEAM + OXC extractors + pipeline + validate, but derived analytics artifacts (coverage, summary, public-exchanges, family-analysis, describe-keys, describe-key-analysis, method-analysis, market-validation) are generated by separate mix tasks and can drift stale after a "full" update. Add these to the update task so a single `mix ccxt_extract.update` refreshes everything.
