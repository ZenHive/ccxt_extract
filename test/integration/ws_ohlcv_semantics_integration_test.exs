defmodule CcxtExtract.WsOhlcvSemanticsIntegrationTest do
  @moduledoc """
  Runs the `ws_ohlcv_semantics` extractor against the real CCXT Pro source.
  ...
  describe "mix ccxt_extract.ws_ohlcv_semantics" do
    test "extracts a well-formed ohlcv-semantics discovery file from real Pro source" do
      output =
        run_task_capturing_output(Mix.Tasks.CcxtExtract.WsOhlcvSemantics, ["--tier1", "--dex"])

      assert output =~ "Extracting WebSocket OHLCV semantics"
      assert output =~ "Done."
      assert output =~ "define handleOHLCV"
      assert output =~ "Output: priv/discoveries/ws_ohlcv_semantics.json"

      data =
        "discoveries"
        |> Path.join("ws_ohlcv_semantics.json")
        |> Paths.out()
        |> File.read!()
        |> Jason.decode!()
      ...
      for id <- present do
        record = WsOhlcvSemantics.build(by_id[id], by_id)
