...
- **Scope-creep verdict applied natively.** A prior audit found the `10-sign-extend` and `sibling-emit` bundles extend closed phases for non-priority exchanges with no `ccxt_client` consumer pull. Both bundles' tasks carry `status = "blocked"` + a `blocked_reason` citing the tier-scoping deferral — revisitable if the priority list expands, not deleted.
- **Tasks with no D/B/U in the source ROADMAP** (127, 66c, 66d, 99, 99b, 24, 36, the rate-limit-header task) — `rmap` structurally requires `scores`, so conservative values were proposed and each task body carries an explicit `PROPOSED SCORES` line flagging it for reviewer confirmation. All are `blocked`, so the proposed scores are not surfaced by `rmap next`.
