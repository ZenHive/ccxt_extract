...
| Task 13 | ⬜ | Universal envelope `tier_scope` stamping [D:2/B:5/U:6 → Eff:2.75] 🎯 — only `_manifest.json` carries `tier_scope` today; other aggregate files (`_base_methods.json`, `_validation_report.json`, `_contract_test_report.json`) don't. Tests currently dispatch on observed counts via `CcxtExtract.Test.ScopeThresholds`; Task 13 stamps all envelopes via `AggregateWriter` so tests can assert on scope directly. Three `TODO(scope-envelope):` markers wait for this — `test/support/scope_thresholds.ex:22`, `test/integration/method_analysis_integration_test.exs:52`, `test/integration/public_exchanges_integration_test.exs:46`. Full spec in `SCOPED-EXTRACTION-TASKS.md` §Task 13. |
