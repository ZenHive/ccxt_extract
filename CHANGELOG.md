...
- **Discovered during rebase verification** — Task 131 (⬜, maintenance) added: 6 seed-dependent flaky tests on `development` (4 in `setup_task_test.exs`, 2 in `contract_test_test.exs`) reproduce identically on upstream HEAD, so they're pre-existing state-pollution from Tasks 115/112, not introduced by Phase 13.
