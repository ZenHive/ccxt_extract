...
- **Pipeline gates:** `mix ccxt_extract.validate` → 110/110 schema-clean, 11/11 round-trip clean. **`mix ccxt_extract.contract_test` reports a regression** — the `error_classes_covered_by_hierarchy` invariant fails for ~20 exchanges (aftermath confirmed; others likely) because `errors.handle_errors.http_exceptions.*` now carries `"__function:AuthenticationError"` / `"__function:ExchangeNotAvailable"` / etc. string values that the flat-parents lookup can't resolve. The `__function:` prefix is the extractor's unresolved-function-reference sentinel — CCXT 4.5.54 evidently changed how some exchanges declare `httpExceptions` in `describe()` (function references where previous releases had class-name strings). Tracked content (this commit) is sound; the regression lives entirely in the gitignored `priv/output/<id>.json`. Needs a follow-up `rmap` task to teach `lib/ccxt_extract/handle_errors.ex` to resolve the function-reference form.
