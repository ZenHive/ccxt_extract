...
### Task 95a — WS orderbook snapshot/delta semantics

Added the `websocket.orderbook_semantics` section to the v4 WebSocket group. A new OXC extractor (`mix ccxt_extract.ws_orderbook_semantics`) scans CCXT Pro `handleOrderBook()` / `handleOrderBookMessage()` (and `handleSnapshot` / `handleChecksum` / `checkOrderBookChecksum`) methods for structurally-resolvable orderbook facts — the snapshot/delta discriminator field + its literal values (bybit `type: 'snapshot'`, okx `action: 'snapshot' | 'update'`, bitmex `action: 'partial'`), the integer-accessor sequence/ordering keys (binance `U`/`u`/`pu`, okx `seqId`/`prevSeqId`), the integrity checksum field + `crc32` algorithm hint, and an `apply_mode` (`incremental` via `handleDeltas` vs `replace` via `.reset()` / `this.orderBook()`) — writing raw, inheritance-free facts to `priv/discoveries/ws_orderbook_semantics.json`. Pipeline assembly resolves `extends`-chain inheritance (a variant/alias Pro class without its own orderbook handler inherits the parent's semantics) and emits an always-present record with `apply_mode`, `handle_orderbook_defined`, `discriminator`, `sequence_fields`, `checksum`, `resolved_from`, `source`, provenance at `/websocket/orderbook_semantics`, JSON Schema coverage (`WebsocketOrderbookSemantics` `$def`), and a `websocket_orderbook_semantics_shape_valid` contract invariant (0 corpus findings). REST-only exchanges carry an honest-empty record (`apply_mode: "none"`, `unresolved_reason: "no_ws_support"`); a Pro class with no orderbook handler carries `no_ws_orderbook`; a handler whose apply strategy is neither incremental nor replace carries `apply_mode: "unknown"` + `unresolved_reason: "orderbook_not_classifiable"` — never a guessed mode. All classification is structural from literal AST arguments, never inferred from server behavior.

### Task 121 — Unified-method descriptors (TS signature + JSDoc overlay)

Added a new OXC extractor (`mix ccxt_extract.method_descriptors`, `CcxtExtract.MethodDescriptors`) that emits one **unified-method descriptor** per CCXT unified method to `priv/discoveries/method_descriptors.json`. Each descriptor fuses two complementary axes, both provenance `raw`: the **TS signature** — ordered params with name, TS type, optional flag, and raw default value, plus return type, read off the method AST — and the **JSDoc overlay** — `@description` prose, per-`@param` prose, `@returns` shape, and `@throws {ErrorClass}` entries. Runtime-verified that OXC surfaces **no** comments on the AST (no `leadingComments`, no program-level `comments` array), so the JSDoc half is recovered with a second pass: a source slice immediately preceding the method node, parsed for tags. Each descriptor also carries `source` — the byte-for-byte method-definition slice via the node's `.start`/`.end` offsets, no normalization — so the descriptor is self-verifying against CCXT source. Descriptors are **consumer-neutral** (no client arg-shape encoded); scope is the structural unified surface (public, non-computed, `!static`, non-`_`, `Promise<…>` return). Partial descriptors are honest: a method with no JSDoc carries `description`/`params_doc`/`returns`/`errors` `null` plus `unresolved_reason: "no_jsdoc"`; a present block missing a tag carries an honest empty value (`errors: []`) rather than `null`. Mix task uses `TaskScope.parse_and_resolve!/2` (per Task 144). Unit tests + doctests cover signature extraction, JSDoc parsing, byte-exact source slicing, honest partials, the merge-safe write, and CLI validation. Unblocks ccxt_client task 109; pipeline (Phase 12) consumption of the discovery file remains downstream.

### Task 78f — parseOHLCV discriminator vocabulary beyond market.inverse

Generalized closed `discriminator` vocabulary in `normalization.field_maps.ohlcv` to include `"market.spot"`. okx's `(type === 'spot') ? 5 : 6` volumeIndex (and transitive bindings) now yields a populated discriminated volume slot (`"discriminator": "market.spot"`, true=spot/5, false=6) instead of `volume=null` + `non_inverse_discriminator`. Unit + cached tests updated; `market.inverse` paths unchanged.

### Task 63 — mix ccxt_extract.drift_audit

Added `mix ccxt_extract.drift_audit` (core `CcxtExtract.DriftAudit`). Report-only comparison of current assembled output (derivation + overrides) against a baseline release snapshot supplied by `--baseline-tag REF` (uses `git show REF:priv/output/<id>.json`) or `--baseline-dir DIR`. Identifies (a) stale overrides (override present and raw/final value differs from baseline), (b) flipped or disappeared derived fields (provenance "derived" and value differs), (c) new raw fields absent from baseline raw (not yet derived). Each finding cites exchange + RFC 6901 pointer + before/after. Exits non-zero only on errors, never on findings. Unit tests with synthetic current/baseline/overrides; mix task follows existing audit-task patterns (`--exchange`, `--report`, summary + bounded sample output). No schema or emitted-JSON change.
...
### Task 127 — Position-aware `paths_rw_split` sinks

Replaced the two band-aids from the Task 112 sanitizer-broadening change with a position-aware sink registry, removing the chop-level masking that both papered over. `ContractTest` now keys writer sinks on `{function, arity} => write_arg_indices` (`@writer_sinks`): a `File` call is a `paths_rw_split` sink only when its `{function, arity}` is registered, and a read-helper path is a leak only when it reaches one of the **write-position** arguments. This distinguishes `File.cp!/2` arg 0 (a read source — a `Paths.priv(...)` path there is fine) from arg 1 (the write destination), so the `pipeline.ex` schema copy reverts from the `File.write!(dest, File.read!(src))` decomposition back to a direct `File.cp!/2`. The position filter also drops control-only reaches (`if File.exists?(p), do: File.write!(other, …)` — the inspected path reaches the writer node through a control edge but never its write-position argument subtree), which let `@file_reader_fns` narrow back to content readers only (`read`/`read!`/`stream!`/`open`/`open!`); path-inspection probes (`exists?`/`stat`/`ls`) are no longer sanitizers, closing the theoretical false negative where a chop-level `File.exists?(p)` sanitizer would also mask a genuine `File.write!(p, …)`. Content readers stay sanitizers so a path that has become file content can still flow into a writer freely. Reachability is computed from the taint result alone — the source node id folded into the chop `path`, intersected against each write-position argument's IR subtree — so no direct `Reach`/`Graph` graph calls are added. New fixture tests cover `File.cp!` read-vs-write position, the `File.exists?`-then-unrelated-write control case, and the `File.read!`-then-write content case. Both Task-127 TODO blocks (in `contract_test.ex` and `pipeline.ex`) are deleted; the real-`lib/` baseline and `mix ccxt_extract.contract_test` stay at 0 `paths_rw_split` findings.

### Task 110 — Triage `request_defaults_resolvable_reachable_from_unified` findings via committed allowlist

The full-corpus baseline surfaced 34 findings for this invariant (e.g. `bybit.fetchSpotMarkets`, `coinbase.fetchAccountsV2`, `binance(us).fetchFundingHistory`). Investigation found two legitimate method classes that carry resolvable literal request bodies yet never appear in `endpoints.unified`: **(1) transitive helpers** (`this.fetchSpotMarkets()` called from a unified `fetchMarkets` — the chain is invisible because `endpoints.unified` *values* store interface names like `publicGetX`, never helper names, and `Pipeline.restrict_to_canonical_vocab` strips the helper *keys*), and **(2) genuine unified methods absent from the filtered map** (`kraken.fetchOrdersByIds`, `binance.borrowIsolatedMargin` — real `has`-true methods whose interface call the unified extractor didn't surface). Option (a) transitive-call analysis would only resolve class (1), so chose option (b): a committed per-exchange allowlist at `priv/contract_test/request_defaults_reachable_baseline.json` (`{exchange_id => [method, …]}`), mirroring the `error_code_fields_roots.json` baseline pattern. `ContractTest.run_all/1` loads it (new `:request_defaults_reachable_baseline` inline + `:request_defaults_baseline_path` opts; missing file → empty allowlist, i.e. strict) and threads it through the check, which exempts only `{exchange, method}` pairs in the baseline — so a helper named for one exchange can't mask a genuine orphan in another, and a *new* unreachable literal method not in the baseline still flags. All 34 entries verified against the CCXT TS source as real helper calls / unified methods. Brings the invariant to 0 findings corpus-wide; the baseline is updated intentionally on CCXT bumps. Also fixed an incidental pre-existing Dialyzer `pattern_match_cov` warning in `Pipeline.prune_dead_bybit_spot_v3_private_interfaces/2` (Task 124 code) — its non-map fallback clause was unreachable (both callers guard `map_size(sigs) > 0`), so it was removed to keep the harness type-check green.
...
### Task 73f — Extend `transaction_classification` to non-unified raw broadcast endpoints

Task 73d's `transaction_classification` was a name-only classifier over CCXT's **unified** method namespace, so consumers could not trust `on_chain == false` as a safety gate — DEX flows that broadcast blockchain transactions through non-unified helpers (`signL1Action`, `starknetSign`, `createSignedRequest`) and implicit-API `sendTx` paths bypassed the convention. This closes that gap.

- **New OXC extractor `CcxtExtract.RawBroadcast`** (`lib/ccxt_extract/raw_broadcast.ex` + `mix ccxt_extract.raw_broadcast`, wired into `ccxt_extract.update`, output `priv/discoveries/raw_broadcast.json`). Parses every method body per exchange and emits `signing_imports` (DEX signing-lib imports — CCXT vendors these as `static_dependencies/noble-curves`; `ethers`/`starknet` markers are forward-looking), `eip712_builder` (whether a method invokes `ethEncodeStructuredData` / `hashTypedData`), and `broadcast_methods` (method → reached helpers). Detection gates: **async** (CCXT API endpoints are async; signing primitives are sync — drops `signL1Action` itself while crediting the endpoints that call it) and **write-side** (`TransactionClassification.transactional?/1`, i.e. not `fetch*` — a `fetch*` that signs is *authenticating*, e.g. paradex signing a starknet challenge to read state, not broadcasting). A transitive-closure fixpoint credits endpoints that sign through an intermediate `build*Sig` helper.
- **`TransactionClassification.derive/3`** now takes `raw_broadcast` and `describe_api`. It promotes every `broadcast_methods` key, plus implicit-API `sendTx` / `sendTxBatch` paths scanned from `describe().api` (map- and array-form), under their CCXT implicit method name (e.g. `publicPostSendTx`). Every promotion sets `on_chain: true` **and** `transactional: true`. Promotion wins over a name-only off-chain base entry.
- **Pipeline wiring**: `raw_broadcast.json` is an optional discovery file in `DiscoveryLoader` + `Pipeline` (parent-fallback for DEX aliases sharing a source class); `Schema.build_exchange` passes it and `describe_api` into `derive/3`.
- **New contract-test invariant** `transaction_classification_promoted_flags_consistent` — flags any entry with `on_chain: true` lacking `transactional: true`, so `on_chain == false`/absence is a trustworthy negative safety gate within detection coverage.
- **Conservative direction**: detection biases toward over-promotion (auth helpers like `signIn`/`onboarding` that share a signing helper are promoted on-chain). Over-promotion never produces a false `on_chain: false`, which is the safe failure mode for a security gate; under-promotion (a missed real broadcast) is the dangerous one and is what the async+write-side+transitive design guards against.
- **Coverage verified** (`:extraction` integration tests): hyperliquid (`signL1Action`/`signUserSignedAction` → `createOrder`/`withdraw`/`setLeverage`), paradex (`starknetSign`, starknet family), grvt (`createSignedRequest` + EIP-712 builder), lighter (`publicPostSendTx`/`publicPostSendTxBatch` from `describe().api`); binance stays name-only (`withdraw` the only on-chain entry). Docs: `SCHEMA.md` + schema `transaction_classification` description + module docs updated.
...
- **`OverrideRegistry.apply_all/2` now translates pointers.** Direct callers passing legacy v3-shaped override entries could have written to `/structure/...` on v4 maps; `apply_all/2` now routes through `translate_pointer/1` so legacy pointer paths land at their v4 destinations.
- **Silent-skip footguns removed in `contract_test`.** `check_normalization_shape_valid` no longer treats a missing `normalization` top-level key as a v3-shape short-circuit (now emits a finding). `check_handler_dispatch_v4_shape_valid` was renamed to `check_handler_dispatch_shape_valid` and the `v4_shape?/1` gate dropped — the gate was dead post-Task-143 and the silent `[]` return could mask extractor regressions on stale gitignored output dirs. `check_rate_limits_endpoint_cost_binding_coherent` got the same treatment.
- **`*_v4` identifier promotion.** `Schema.build_exchange_v4/4` → `build_exchange/4`, `Schema.validate_v4/1` → `validate/1`, `Schema.check_schema_version_v4/2` → `check_schema_version/2`, and every `@required_*_v4` module attribute lost the now-redundant suffix. The Provenance v4 names had already been promoted in fa64144; this pass extends that to Schema + Validation. Updates the prior Task-143 CHANGELOG bullet which had been narrowly accurate (only the `schema_*_for/1` and `schema_*_v4/0` lookup quartet were dropped at first) — the broader rename now covers `build_exchange_v4` and `validate_v4`.
- **`sign_recipe_v1.json` description fix.** Stopped referencing the deleted `exchange_v3.json` and the deleted `sign_recipe_test.exs` parity test; description now points at `exchange_v4.json#/$defs/SignRecipeRecord` with JSV-time lockstep verification.
- **Test scaffolds cleaned.** `request_shape_cached_test.exs` dropped the TODO(Task 143) v3-fallback chain and `v4_or_v3_pointer/1` helper. `authenticated_sections_integration_test.exs` updated `runtime.describe.api` / `structure.*` reads to `raw.describe.api` / `auth.*` (the test was silently no-op on v4 output). `schema_v4_emit_cached_test.exs` dropped the five ignored `schema_target: 4` opts.
- **`handle_errors.ex` sentinel dedupe.** `strip_function_sentinel_value/1` and `normalize_class_name/1` now both read from a single `@function_sentinel` module attribute (defined once at module top). Added list-traversal clause for defensive coverage of any future shape change; the existing single-strip behavior on `"__function:__function:Foo"` is preserved by design and documented inline.
...
- `CcxtExtract.Currencies` also drops QuickBEAM's `"__undefined"` sentinels recursively — JS `undefined` fields (e.g. `precision`, `active` on exchanges that don't surface them) were leaking into the emitted JSON as that literal string and failing the typed `Currency`/`NetworkInfo` schema. An absent key is the lightweight, schema-conformant encoding of "field not surfaced".
- `Currency.precision` / `NetworkInfo.precision` schema widened to `number | string | null`: hyperliquid emits currency precision as a decimal string (e.g. `"0.00000001"`) to preserve exact tick sizes. Extracted verbatim rather than coerced — matches the existing `fee` field's number-or-string union and avoids float-representation drift.
...
- **Universe delta:** `+weex`, `−zonda`; total stays at 110.
- **`priv/discoveries/class_hierarchy.json`** regenerated. Large diff (+1684/−398) reflects upstream method growth across 6 patch releases — e.g. `aster` went 71 → 84 methods; method `statements` counts shift across most exchanges. No structural schema change; the `@external_resource` consumer in `tiers.ex` recompiles cleanly.
- **`priv/fixtures/signing/*.json`** fully regenerated (107 entries) — frozen credentials/timestamps/nonces unchanged, so diffs reflect only CCXT-side wire-shape evolution. `_manifest.json` updated; new `weex.json`; `zonda.json` removed.
- **Pipeline gates:** `mix ccxt_extract.validate` → 110/110 schema-clean, 11/11 round-trip clean. **`mix ccxt_extract.contract_test` surfaced a normalization gap** — the `error_classes_covered_by_hierarchy` invariant fails for ~18 exchanges because `errors.handle_errors.http_exceptions.*` carries `"__function:AuthenticationError"` / `"__function:ExchangeNotAvailable"` / etc. sentinels that the bare-string flat-parents lookup can't resolve. CCXT's `httpExceptions` declares values as bare JS class identifiers (`'422': ExchangeError`, not the string `"ExchangeError"`); QuickBEAM serializes those as `"__function:<ClassName>"`. The shape is stable across CCXT releases — this is an extractor-side gap, not an upstream change. Tracked content in this commit is sound (the sentinels live entirely in the gitignored `priv/output/<id>.json`); the fix landed in the follow-up commit (3731d52) by stripping the `__function:` prefix at the `handle_errors.ex` extraction boundary. Residual 2 `error_classes_covered_by_hierarchy` findings (`apex.exceptions.exact["20006"]`/`["20016"]`) are unrelated — apex ships human-readable error messages (`"apikey sign error"`, `"request para error"`) in the class-name slot; separate `priv/overrides/apex.json` task.

### Task 142 — v4 schema cut: `exchange_v4.json` is now the default

v4 (`exchange_v4.json`, `schema_version` `"4.0.0"`) is now the default emitted and validated schema. `mix ccxt_extract.pipeline`, `mix ccxt_extract.update`, and `mix ccxt_extract.validate` all target v4 without any flag. The `runtime` / `structure` top-level sections have been reshaped into consumer-shaped groups — `endpoints`, `auth`, `errors`, `rate_limits`, `normalization`, `markets`, `testnet`, and `raw` — per the path-migration table in `SCHEMA.md` § "Top-level reshape". v3 remains reachable via `--schema-target=3` as a legacy fallback and is removed in Task 143. Consumers should migrate by bumping their major-version pin from `3` → `4`, updating top-level key reads, and pointing any JSON-Schema integration at `exchange_v4.json`.

### Roadmap migration — `ROADMAP.md` is now generated by `rmap`

- **`roadmap/tasks.toml`** (new) — the typed source of truth for the roadmap, consumed by `rmap`, the single-binary Rust roadmap CLI. Holds the open task set (every `pending` + `blocked` task) plus the phase/bundle skeleton; completed-phase history stays in `ROADMAP.md`'s preserved "Completed Phases" prose and the entries below. `rmap render` regenerates `ROADMAP.md` + `roadmap/data.json` from it; `rmap validate --check-render` gates drift.
- **`roadmap/data.json`** (new, generated) — the agent-facing view of the same data. Committed, not gitignored — read-by-agents derived state, same handling as `priv/discoveries/class_hierarchy.json`.
- **`ROADMAP.md` restructured into a generated view.** Per-phase task tables now live between `<!-- TASKS:BEGIN phase=N -->` / `<!-- TASKS:END -->` marker pairs and the Current Focus block between `<!-- FOCUS:BEGIN/END -->`. All prose outside the markers (Vision, Endpoint-Invocation Priority Order, v4 Schema-Freeze Plan, phase-intro blockquotes, Consumer Architecture, Notes) is byte-preserved across `rmap render`. **Do not hand-edit `ROADMAP.md`** — edit `roadmap/tasks.toml` and re-render.
- **Open-tasks-only reorg.** Under rmap's one-table-per-phase model, done-task rows leave the phase tables — completed work lives in this changelog and the preserved "Completed Phases" prose. Fully-done Phases 10 and 14 lost their dedicated sections (absorbed into "Completed Phases"); "Superseded / Deferred" shrank to just the `⛔` truly-dead items, since `🔶`-deferred tasks now render in their phase tables as `blocked`. This *reduces* drift surface — flagged here so the diff isn't surprising.
- **Duplicate task IDs resolved.** The hand-maintained file had four colliding IDs (57c, 113, 132, 133). The two **audit-surfaced** tasks that collided with Phase 13's `132`/`133` were renumbered — `139` (docs-drift: reconcile AGENTS.md cloud-agent guidance) and `140` (test-coverage: pipeline-level `endpoint_cost_binding` integration tests); `138` is the real `det-contract` task added to `development` by the post-`f3ea824` audit commit, picked up when this branch rebased. Phase 13's `132`/`133` keep their IDs; the `⛔` LFS-`113` was already superseded-and-resolved and is not migrated.
- **Scope-creep verdict applied natively.** A prior audit found the `10-sign-extend` and `sibling-emit` bundles extend closed phases with no `ccxt_client` consumer pull (each task's `blocked_reason` says "no priority consumer" — not that the exchanges are non-priority; several name Tier 1/2 exchanges). Both bundles' tasks carry `status = "blocked"` + a `blocked_reason` citing the tier-scoping deferral — revisitable if the priority list expands, not deleted.
- **Tasks with no D/B/U in the source ROADMAP** (66c, 66d, 99, 99b, 24, 36, the rate-limit-header task) — `rmap` structurally requires `scores`, so conservative values were proposed and each task body carries an explicit `PROPOSED SCORES` line flagging it for reviewer confirmation. All are `blocked`, so the proposed scores are not surfaced by `rmap next`.
- **Convention docs updated.** `CLAUDE.md` § "Documentation invariants" item 1 now points status flips at `roadmap/tasks.toml` + `rmap render` instead of direct `ROADMAP.md` edits; the intro line flags `ROADMAP.md` as generated. `lib/ccxt_extract/request_defaults.ex`'s stale `ROADMAP.md:218` line-number reference now cites the section by name (line numbers in a generated file drift).
- **Residual friction (not fixed here).** `staged-review:audit-review` writes post-merge status flips directly into `ROADMAP.md`; under rmap those get rendered away on the next `rmap render`. The skill is upstream — the mitigation is the `CLAUDE.md` convention edit documenting that flips go through `tasks.toml`. `AGENTS.md` (auto-generated from `CLAUDE.md` by an out-of-repo marketplace script) was deliberately not hand-edited: its Documentation-invariants section inherits the `CLAUDE.md` change on the next sync, and its cloud-agent-flow drift is already tracked as Task 139.

### Task 114 — Extraction determinism audit

- **`mix ccxt_extract.determinism_check`** (`lib/mix/tasks/ccxt_extract.determinism_check.ex`) — new Mix task. Runs a list of extraction tasks twice into two isolated tmp dirs (via `:priv_write_override`), then byte-diffs every `.json` file produced. Volatile timestamp keys are stripped and both sides are re-encoded through sorted-key canonical JSON before comparison, so the "byte equal" claim means *canonical-form bytes match* — map-iteration order and wall-clock stamps can't masquerade as drift. Exit 0 = all files byte-identical; exit 1 = any divergence or read/decode error. Flags: `--task` (repeatable), `--scope-args="..."`, `--strip-keys`, `--diff-dirs`, `--context-bytes`.
- **`CcxtExtract.JsonDiff`** (`lib/ccxt_extract/json_diff.ex`) — new module backing the harness. `diff_files/3` decodes both files, strips volatile keys at every depth, canonical-encodes, and returns `:equal | {:diff, context} | {:error, reason}` with a byte-offset divergence preview. `default_volatile_keys/0` is the shared strip list.
- **Deep key sort in `AstNormalize.to_encodable/1`** — object keys are now sorted recursively before JSON encoding. Map iteration order through `Jason.encode!` was a silent non-deterministic source: the same extraction produced different byte output across runs purely from map-traversal order. This is the load-bearing fix — it makes output deterministic at the source, not just papered over in the checker.
- **`Pipeline.check_version_drift!/1`** — a loud-fail guard run at the top of `Pipeline.extract/1`. Aborts the pipeline with a resync hint when `priv/ccxt` HEAD or `priv/ccxt_bundle.js` no longer matches the baseline recorded in `priv/ccxt_version.json`. Silent CCXT version drift previously meant the corpus could regenerate against a different upstream with no signal. Bypass: `--allow-version-drift` on `mix ccxt_extract.pipeline` (threads through to `extract/1` as `allow_version_drift: true`). Missing version file or `source_git_sha: "unknown"` is a skip, not a failure — there is no baseline to drift from. New `bundle_sha256/1` helper computes the lowercase-hex sha256 the guard compares against; `mix ccxt_extract.setup` now records `bundle_sha256` into `priv/ccxt_version.json`.
- **Verification.** `mix ccxt_extract.determinism_check` for `--tier1`, `--tier1 --tier2 --dex`, and `--all` all report 0 diverged across consecutive runs (13/13, 26/26, 114/114 byte-identical). Version-drift guard verified loud-fail + bypass via `test/ccxt_extract/version_drift_test.exs` and a corrupted-`source_git_sha` smoke test.
- **Why this matters.** Untracking the derived corpus (2026-04-18) stopped the repo-size bleeding but left the root cause — every `mix ccxt_extract.update` produced ~110-file diffs even when upstream CCXT was unchanged. With extraction now byte-deterministic for a fixed CCXT version + bundle + scope, freeze diffs are meaningful and the v4 emit-default soft gate (Task 114) is cleared.
- **Follow-ups recorded.** Task 136 (re-track `priv/output/` now that diffs are meaningful) and Task 137 (retrofit Pattern B timestamp writers to accept an `:extracted_at` opt so `determinism_check` can pass a frozen clock instead of stripping keys).
- **🔗 Cross-repo:** `../ccxt_client/ROADMAP.md` Task v4-adopt's determinism soft gate is now cleared — it remains 🔶 Blocked only on the freeze list emptying, no longer on Task 114.

### Task 83b — Response envelopes derivation (Phase 12 freeze-gate cleared)

- **`CcxtExtract.Normalization.ResponseEnvelopes`** (`lib/ccxt_extract/normalization/response_envelopes.ex`) — new module. `derive/2` consumes the per-exchange `parse_methods.json` entry (carrying `parse_dispatch`) and the per-exchange `fetch_methods.json` entry (Task 83a's fetcher body slice). Returns a per-parser-type map keyed by `ticker` / `trade` / `ohlcv` / `order` / `position` / `balance` / `market` / `transaction` / `deposit_address`, each populated with a per-fetcher sub-map of `%{"key", "fallback_keys", "default"}` envelope records (or an honest `_unresolved_reason` entry).
- **N:1 fetcher→parser shape.** Recognized as the rule, not the exception — binance's `parseTrades` is reached from `fetchTrades`, `fetchMyTrades`, and `fetchMyDustTrades`, each with a different envelope. The populated entry is therefore a per-fetcher map, not a single key.
- **AST walker.** Walks each fetcher's `BlockStatement` body looking for `this.safeValue(response, "<KEY>", <DEFAULT>)` / `this.safeList(...)` / `safeValue2` / `safeListN` (closed vocab) where the first argument resolves to the fetcher's `response` binding. First such call is the envelope unwrap; key + fallback keys + default literal are recorded. `fallback_keys` is always a list (may be empty); `default` is the literal third arg (number, string, list literal, object literal, or `null`).
- **Fetcher scope filter.** `fetchers_for_type/2` filters `parse_dispatch` entries to names beginning with `fetch` — mutators like `createOrder` / `cancelOrder` / `editSpotOrder` are excluded even when they share a parser callee. Fetchers that intersect a parser-type's parse_fn set are kept; fetchers that don't dispatch to the parser type are dropped.
- **Closed-vocab `_unresolved_reason` strings:** `"no_fetcher_method_body"` (no entry in `fetch_methods.json`), `"no_safe_value_call"` (body has no `safeValue` / `safeList` call against `response`), `"non_literal_key"` (first key arg is a variable), `"nested_response_unwrap"` (first arg is a sub-property access like `response["foo"]["bar"]`).
- **Carrier wiring.** `Normalization.build/2` now threads `fetch_methods_entry` through to `response_envelopes_record/2`, which `Map.merge`s the stub with derived output; `_unresolved_reason` flips from `"not_yet_derived"` → `nil` whenever any parser-type slot resolved. `build/1` shim preserved for callers (tests + `Schema.build_exchange_v4/4` fallback) that don't carry a `fetch_methods_entry` — response envelopes degrade to the stub on that path.
- **Pipeline + discovery loader.** `fetch_methods.json` made optional throughout — missing-files raise filters it out, `:fetch_methods` key reads via `Map.get(data, :fetch_methods, %{})` so test fixtures without the key still build cleanly.
- **Corpus coverage (verified via `mix ccxt_extract.update --schema-target 4`).** binance.json `.normalization.response_envelopes.trade` shows three fetcher keys: `fetchMyDustTrades` carries `key: "userAssetDribblets"` (the documented two-level unwrap), `fetchMyTrades` + `fetchTrades` resolve to `key: null` (response IS the array, no unwrap).
- **Phase 12 v4 schema-freeze gate cleared.** With Task 83b shipped, every normalization-bundle freeze-gating task (Task 129 carrier · Tasks 74–82 field maps · Task 83a fetcher extractor · Task 83b envelopes) is ✅. Phase 12 retains the non-gating edge-case tasks (78c hybrid `Array.isArray`, 78d scrambled-coercion, 78f okx multi-market discriminator, 135 ticker.ex vocab alignment) — none gate v4 emission.
- **No schema bump** — `NormalizationStubValue` is `additionalProperties: true`; existing v4 schema validates the populated map cleanly.
- **🔗 Cross-repo:** `../ccxt_client/ROADMAP.md` Task v4-adopt remains 🔶 Blocked on Task 114 (extraction determinism audit, parallel session). Once 114 lands green, the v4 emit default flips and v4-adopt unblocks.
- **Audit follow-up (`audit(b860b44)` — Codex dual-reviewer pass).** Closed five corpus-data-loss gaps in `response_envelopes.ex`, all verified against the live CCXT corpus before applying. (F1) `@parser_type_to_parse_fns` was missing the plural parsers `parseTickers` / `parseOHLCVs` / `parseDepositAddresses` — every exchange implementing only the plural fetcher resolved those slots to `null` (binance/okx ticker was the load-bearing victim). (F2) inline `return this.parseX(this.safeList(response, "k"), …)` calls — no intermediate `const` — fell through to unresolved; `find_return_first_arg/1` now classifies the inline `safe*` call via a new `{:inline_binding, _}` tuple (whitebit/kraken/bitget). (F3) direct member-access returns `return this.parseX(response['k'], …)` / `response.k` now resolve the property as the envelope key (bitso/coinex/coinmate/bittrade/zaif). (F4) `safeValue2` / `safeList2` arity bug — a 3-arg call with no default recorded the fallback key `k2` *as* the default and left `fallback_keys` empty; `extract_key_binding/2` is now method-aware via `parse_remaining_args/2` (zonda). (F5) top-level `_unresolved_reason` was hardcoded `nil` even when `parse_dispatch` had only mutator entries (no `fetch*` caller) — now emits the new closed-vocab string `"no_fetcher_dispatch"` (binanceus/fmfwio/kucoinfutures), added to the moduledoc, `SCHEMA.md`, and the cached-test allowlist in lockstep. +12 regression tests (one describe block per finding). Also regenerated `.sobelow-skips` — b860b44's own `pipeline.ex` edit shifted the `File.mkdir_p!` / `File.write!` line numbers that the skip file fingerprints by line. Reports: `.audit/b860b44-task-83b-response-envelopes.md`, `.audit/6854307-task-83a-fetch-methods.md` (clean), `.audit/4e235f3-claude-md-import-drop.md` (clean, fast-path).
...
- **Audit follow-up (`1fc712f` + Codex follow-up audit).** Closed three honesty-contract gaps from the post-merge audit. (a) `market.ex` now ships an `unwrap_ts_as/1` helper (recursive over chained `as Foo as Bar` casts) and a `classify_return_argument/1` head-set; unblocks `grvt` which previously fell through as a misleading `"no_return_statement"`. (b) `balance.ex` gained an explicit `identifier_return` clause for bare-Identifier returns (lbank); the wildcard fall-through is renamed `unrecognized_return_shape` so consumers no longer see "no return" when a return statement is in fact present. (c) `transaction.ex` + `deposit_address.ex` `unwrap_ts_as/1` helpers made recursive for template parity. Moduledoc vocab + `SCHEMA.md` ticker/market/balance sections list all `_unresolved_reason` values exhaustively; `non_object_return:unknown` documented as the "argument has no `type` field" edge case. Schema permissive (`additionalProperties: true`) — no bump. Sibling drift in `ticker.ex` deferred as Task 135.
...
- **13 unified fields** — `id`, `timestamp`, `datetime`, `symbol`, `order`, `type`, `side`, `takerOrMaker`, `price`, `amount`, `cost`, `fee`, `info`. Always emitted as keys with `null` value when absent or outside the closed vocab. Three are structurally-null by design: `symbol` (from market argument, not raw), `datetime` (derived from `timestamp`), `info` (bare pass-through). `cost` is a scalar field but typically null in the current corpus because CCXT computes `price × amount` downstream when the parseTrade body omits it.
- **Enum fields** — `type`, `side`, and `takerOrMaker` carry a new `enum_map` slot. Populated from: `safeStringLower` (canonicalizing extraction → `enum_map: nil`, passthrough), explicit `safeString().toLowerCase()` chains (canonicalized), and ConditionalExpression chains over a shared safe-call (`safeString === 'T' ? 'taker' : (safeString === 'M' ? 'maker' : undefined)` → `%{"T" => "taker", "M" => "maker"}`). Boolean/numeric/char-code ternaries emit slot with `enum_map: nil` and per-slot `unresolved_reason`.
- **Nested `fee` field** — carries a new `sub_field_map` slot (vs flat `key`/`coercion`/`format`). Resolves ObjectExpression literals (`{ cost: this.safeNumber(...), currency: this.safeCurrencyCode(...) }`) into `%{"cost" => slot, "currency" => slot}`. `safeCurrencyCode` is a 1-arg vocab member; when bound to an Identifier (`const feeCurrencyId = this.safeString(trade, 'feeCcy')`), the slot traces the binding chain back to the underlying safe call's wire key. Non-ObjectExpression fees (variable references, externally-built objects) emit `sub_field_map: nil` with `unresolved_reason: "fee_not_object_literal"`.
- **Multi-payload detection** — parseTrade bodies that dispatch on shape (`Array.isArray(trade)`, `'someKey' in trade`, `typeof trade === 'string'`) at the top-level IfStatement emit a single honest `_unresolved_reason: "multi_payload_branching:<N>"` rather than committing to one shape. Binance's `parseDustTrade` delegation guard (`if ('isDustTrade' in trade) { return this.parseDustTrade(...) }`) classifies as multi-payload.
- **Extended coercion vocab** — adds `safeStringLower` (enum canonicalizer) and `safeCurrencyCode` (1-arg currency resolver) on top of the existing safeString family / safeNumber / safeInteger / safeTimestamp set used by ticker/ohlcv.
- **Wired through `Normalization.build/2`** — `field_maps_record/1` now calls `Trade.derive(parse_methods_entry)` alongside `OHLCV.derive/1` and `Ticker.derive/1`.
- **Corpus coverage** — okx (canonical safeTrade return, 7 of 13 fields populate — `cost`/`datetime`/`info`/`symbol` are structurally null by design), deribit (canonical safeTrade return), binance/kraken (multi-payload bodies emit honest `multi_payload_branching:<N>` tags).
- **Schema permissive** — `NormalizationStubValue` is `oneOf null / object` with `additionalProperties: true`, so the new `enum_map` and `sub_field_map` keys validate without a schema bump. v4 schema validation passes for all 4 priority exchanges.
...
- **Bitmex bare-Identifier volume.** Bitmex's volume element is a bare `Identifier` (`volume`) bound at the top of the body to `this.convertFromRawQuantity(...)`. The slot is honest-null with reason `volume:non_safe_coercion:convertFromRawQuantity`. Two adjacent fallback reasons distinguish related shapes: `volume:non_call_init` (identifier bound to a non-call expression), `volume:volume_index_unbound:NAME` (identifier not bound in the body at all).
- **parse8601 wrapper (78e).** Bitmex's timestamp is wrapped as `parse8601(safeString(ohlcv, key))`. The extractor now recognizes this shape for the timestamp slot only (parse8601 anywhere else falls through to `non_call_element` rejection). Resolved slots carry `"coercion": "parse8601"` and `"format": "iso8601"`. The closed coercion vocabulary expands by one; the `@parse_iso ~w(parse8601)` module attribute in `ohlcv.ex` is the registry.
...
- **No schema bump.** `NormalizationStubValue` in `priv/schema/exchange_v4.json` is structurally unconstrained (`oneOf [null, object additionalProperties: true]`). The new keys (`key`, `format: "iso8601"`, `input_shape`) land freely without a `$defs` change or version increment. Schema version (at time of this task) was `"4.0.0-pre"` — now `"4.0.0"` as of Task 142.
...
- **`CcxtExtract.Normalization.OHLCV`** — new module (`lib/ccxt_extract/normalization/ohlcv.ex`). `derive/1` projects a per-exchange `parse_methods.json` entry into `field_maps["ohlcv"]` for exchanges whose `parseOHLCV` body is a single `ReturnStatement` with an `ArrayExpression` of `this.safe{Integer,Number}{,2}` calls. Closed coercion vocabulary `["safeInteger", "safeInteger2", "safeNumber", "safeNumber2"]` — anything else emits the slot as `null` with a closed-vocab `_unresolved_reason`.
- **Output shape.** A populated record carries one branch with `guard: %{"kind" => "always"}`, `shape: "array"`, and a `field_map` keyed by `timestamp/open/high/low/close/volume`. Each pure slot is `%{"index", "key", "coercion", "format"}` (timestamp gets `format: "ms"`; others `nil`). When `volume`'s index resolves through `volumeIndex = inverse-test ? a : b`, the slot is a `%{"kind" => "discriminated", "discriminator" => "market.inverse", "true" => slot, "false" => slot}` record. Unresolvable slots are `null` with a per-branch reason.
- **Wired through `Normalization.build/2`.** Replaces the `nil` stub at `field_maps["ohlcv"]` with `OHLCV.derive(parse_methods_entry)`. Other eight parser-type slots stay `nil` (their tasks 74–77, 79–82 ship next). `response_envelopes` untouched (Task 83).
- **Honesty rule.** Every populated slot is provable from AST. `okx`'s `parseOHLCV` ships with `volume = null` plus `_unresolved_reason: "...non_inverse_discriminator..."` because its `volumeIndex` is gated on `(type === 'spot') ? 5 : 6`, not `market.inverse` — generalizing the discriminator vocabulary is deferred to a Phase 12 follow-up rather than expanding scope here. Inheriting exchanges (no `parseOHLCV` override) emit `null` at the carrier slot — the per-method digest's absent entry is the honest signal.
- **Bot-review hardening (post-initial-commit).** Walker now collects ALL `ReturnStatement` arguments (not just `ArrayExpression`-shaped ones) so a hybrid `if (Array.isArray()) return [...]; return {...}` body classifies as `ambiguous_return_shape` instead of fabricating an always-array map from the single array branch. Collector stops at function-body boundaries (`FunctionDeclaration`/`FunctionExpression`/`ArrowFunctionExpression`) so a nested callback's array return cannot pollute the parser's own count. `>6`-element arrays (kraken VWAP-extras shape) fail closed with `extras_not_supported:N_elements` instead of silently mismapping the 6th element to `volume`. New reason `non_literal_index:<method>` distinguishes "in-vocab method, non-literal index" from "out-of-vocab method" — consumers were previously unable to dispatch correctly because both surfaced as `non_safe_coercion`.
- **Schema.** No bump. `NormalizationStubValue` in `priv/schema/exchange_v4.json` already accepts `oneOf [null, object additionalProperties: true]`. SCHEMA.md gains an explicit "closed-vocabulary exception to file-wide open-enum rule" callout for `coercion` and `discriminator` so consumers know unrecognized values are hard errors, not forward-compat fallbacks.
- **Provenance.** No change. `@derived_pointers_v4` already covers `/normalization/field_maps` (added under Task 129).
- **Contract invariants.** No change. `normalization_shape_valid` accepts both `nil` and `map()` at slot level; `parse_methods_digest_covers_inventory` is unaffected.
- **In-scope exchanges this PR.** `binance`, `bybit`, `bitget` (discriminated volume via `market.inverse`), `bitfinex`, `aster` (pure-direct, includes bitfinex's scrambled `[ts, open, high@3, low@4, close@2, vol]` order). `okx` populates 5/6 slots with the volume slot honest-null. Object-shape (`htx`, `bitmex`, `hyperliquid`, `lighter`), hybrid `Array.isArray` (`gate`), and scrambled-coercion (`coinbaseexchange` `safeTimestamp`, `kraken` VWAP-extras, `kucoin` heuristic) are explicit follow-ups (Tasks 78b/78c/78d).
- **Files modified.** `lib/ccxt_extract/normalization/ohlcv.ex` (new); `lib/ccxt_extract/normalization.ex` (alias + `field_maps_record/1` private wiring); `test/ccxt_extract/normalization/ohlcv_test.exs` (new; 27 cases against synthetic AST including hybrid-return / nested-fn / >6-element / non-literal-index regressions); `test/ccxt_extract/normalization_test.exs` (positive assertion that ohlcv slot populates when `parseOHLCV` is supplied); `test/integration/cached/schema_v4_emit_cached_test.exs` (per-exchange shape assertion in the priority loop — coercion + guard kind + discriminator only, indices not asserted because they drift with upstream CCXT).
- **Cross-repo.** `../ccxt_client/ROADMAP.md` Tasks 19/21/44/55 — `field_maps.ohlcv` is now populated for the listed exchanges; consumer-side OHLCV normalization can begin against the documented shape. Tasks 19/21/44/55 stay open until the consumer adopts the spec.
- **Follow-ups opened.** Task 78b (object-shape `parseOHLCV`), 78c (hybrid `Array.isArray`), 78d (scrambled-coercion + heuristic), 78e (`parse8601` ISO8601 timestamps), 78f (extend discriminator vocabulary beyond `market.inverse` — surfaces from okx and likely future exchanges with multi-market-type gating).
...
- **`CcxtExtract.HandleErrors.http_status_map/1`** — derives `%{<HTTP status string> => [%{class, source} ...]}` from two channels: (a) `describe().httpExceptions` (lifted as `source: "http_exceptions"`) and (b) literal `throw new <X>(...)` IfStatement predicates whose innermost test matched `code === N` / `code <op> N` / `||`-disjunctions thereof (lifted via `error_dispatch[predicate_kind == 'http_status_in']`, `source: "throw_dispatch_predicate"`). Both channels coexist for drift detection; same status mapping to the same class from both sides is honest agreement, divergence is the signal.
- **`CcxtExtract.HandleErrors.retryable_buckets/1`** — bucketed retry classification: `rate_limit` (DDoSProtection/RateLimitExceeded), `auth` (AuthenticationError), `server_busy` (ExchangeNotAvailable/OnMaintenance/BadResponse/NullResponse), `network` (NetworkError minus rate_limit), `non_retryable` (everything else). Class names sourced from `http_exceptions` + `exceptions` (broad/exact + market-type-specific) + `error_dispatch[].exception_class` (literal throws). Buckets walked through `CcxtExtract.ErrorHierarchy.bucket_for/1`, which uses the canonical CCXT taxonomy as ground truth. Empty buckets ship as `[]` — the shape is constant.
- **`CcxtExtract.ErrorHierarchy`** (new) — corpus-global `hierarchy/0` map keyed by exception class name, with `bucket_for/1` mapping any class to one of the 5 retry buckets. Module is a parser-walk over CCXT's `priv/ccxt/ts/src/base/errorHierarchy.ts`, materialized at compile time so retry classification doesn't pay file-read cost per exchange.
- **Schema** — both v3 and v4 bumped. v3 → **3.4.0** (additive: 2 new required keys `structure.error_status_map` + `structure.error_retryable`; description threads cumulatively from 3.0.0 onward; `const "3.4.0"`). v4 → unchanged version (no const yet, additive within the existing `errors` group: `errors.status_map` + `errors.retry_classification` join existing `errors.handle_errors` + `errors.class_hierarchy`). New `$defs.ErrorStatusMap`, `$defs.ErrorRetryable` referenced from both schemas. `ErrorClassHierarchy` `$def` is the rich `{tree, flat_parents, ancestors}` form on both.
- **Provenance** — six new `derived` pointers: `/structure/error_status_map`, `/structure/error_retryable`, `/endpoints/handlers/error`, `/endpoints/handlers/signing`, `/endpoints/handlers/parse`, `/errors/status_map`, `/errors/retry_classification`. All projections over already-extracted raw fields (`handle_errors`, `error_dispatch`, etc.) — no new raw-extraction surface.
- **Contract invariants** — two new corpus-level checks: `handle_errors_retryable_shape_valid` (per-exchange shape gate on `error_status_map` + `error_retryable`; both nullable, null short-circuits) and `handler_dispatch_v4_shape_valid` (v4-only — gates the new `endpoints.handlers.{error,signing,parse}` projections). Pre-existing `error_class_hierarchy_shape_valid` invariant unchanged. Drift between the same status mapping to different classes from `http_exceptions` vs `throw_dispatch_predicate` channels is the signal a future Task 132 will turn into a finding.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` Task 58 (adopt error contract) — Phase 13 classify is **complete** as of this PR (Tasks 85+86+87 all ✅; dispatch already complete via PR #6). Consumer-side adoption can now proceed against the full classify+dispatch surface.
- **Follow-up tasks** — three review-time discoveries opened against Phase 13: Task 132 (split `predicate_kind: "http_status_in"` into `eq` vs `range` — current single-bucket lumping loses information for retry/backoff heuristics), Task 133 (explicit `error_class_hierarchy` content-equality invariant — today only outer shape is enforced, drift would only surface as a JSV failure on cosmetic changes), Task 134 (thread precomputed `error_dispatch` through `http_status_map/1` and `retryable_buckets/1` — both helpers re-walk the AST when callers already have the predicate-classified dispatch list). Plus three bot-finding fixes from review (numeric-key filter on `httpExceptions`, recursive walk for 3-level market-type-keyed `exceptions` like binance USDM/COINM + bybit linear/inverse + okx variants, docstring tightening on `check_handle_errors_retryable_shape_valid`) — landing as a follow-up PR against `development`.
...
- **`CcxtExtract.RateLimitBuckets`** — QuickBEAM extractor. Instantiates each non-alias CCXT exchange so `Exchange.initRestRateLimiter()` runs (per `priv/ccxt/ts/src/base/Exchange.ts:339-345 + 3531-3553`), then reads the resolved `this.rateLimit` / `this.rollingWindowSize` / `this.rateLimiterAlgorithm` / `this.tokenBucket` instance fields and assembles `%{"buckets" => [...], "source" => "describe", "unresolved_reason" => nil}`. Bucket entries shape: `{axes, rate_limit_ms, refill_per_sec, max_size, cost_default, algorithm, rolling_window_ms}`. Today every exchange resolves to exactly one bucket (the default request-rate throttle); the `buckets` wrapper ships as a list to leave room for multi-bucket exchanges (binance UID/IP, okx per-account) without a schema break. Pure-Elixir `derive/1` twin runs the same projection over a captured-snapshot map for unit tests.
- **`mix ccxt_extract.rate_limit_buckets`** — corpus-level mix task with standard `--tier1/--tier2/--tier3/--dex/--all/--exchange` scope flags via `CcxtExtract.TaskScope`. Writes to `priv/discoveries/rate_limit_buckets.json` through `CcxtExtract.AggregateWriter` (scoped runs merge with existing aggregate, count recomputed from final entries). Wired into `mix ccxt_extract.update`'s `@default_quickbeam_extractors` orchestrator step; `CcxtExtract.DiscoveryLoader` now loads the file and `CcxtExtract.Pipeline.build_exchange_data/3` reads it via `get_rate_limit_buckets/2` with parent-class fallback.
- **Schema** — additive on both surfaces. v3: new `$defs.RateLimitBuckets` and `$defs.RateLimitBucketEntry` referenced from `structure.rate_limit_buckets` (oneOf null/wrapper). v4: new `RateLimitBuckets` + `RateLimitBucketEntry` `$defs` referenced from `rate_limits.buckets`; the v4 `RateLimits` group now exposes both `buckets` (Task 89) and `per_endpoint_cost` (Task 73) as additive sibling fields. `unresolved_reason` closed vocabulary: `"instantiation_failed"` (constructor threw mid-`describe()` lookup), `"rate_limit_unset"` (rateLimit missing/-1 — base CCXT requires it but a misconfigured override could still emit the field), `"no_discovery_entry"` (Elixir-side pipeline fallback when `priv/discoveries/rate_limit_buckets.json` has no row for the exchange — the JS extractor never emits this reason).
- **Provenance** — `/structure/rate_limit_buckets` (v3) and `/rate_limits/buckets` (v4) tagged `derived`. The wrapper is a projection over CCXT runtime fields, not a raw mirror.
- **Honesty Rule.** `empty_record/0` always emits the wrapper shape with `unresolved_reason: "no_discovery_entry"` rather than `null`-ing the slot — consumers can pattern-match the reason without a presence check. JS-side `extractRateLimitBuckets` follows the same shape: thrown constructors emit `unresolved_reason: "instantiation_failed"` with `buckets: []` rather than aborting the per-exchange entry.
- **Multi-axis caveat (out of scope, deliberately).** CCXT's runtime throttles every request through a single resolved `tokenBucket` regardless of the exchange's wire-level multi-axis policy (binance UID/IP, okx per-account, kraken counter increments). Capturing those wire-level distinctions would require parsing exchange API responses, well outside this extractor's surface. The schema leaves room for a later task to promote `axes: ["request"]` to `["uid", "ip"]` etc. additively without a schema break.
- **Review fix-up commit (`a2b660f`)** — Tier-2 review applied 6 findings locally in worktree `~/_DATA/worktrees/ccxt_extract/task-89-fix/`, pushed directly to the cursor branch under the worktree-as-scope authorization (Cursor Cloud retired, per project CLAUDE.md). The P1 was a real regression: `mix ccxt_extract.update` raised on a fresh clone because `DiscoveryLoader` requires `rate_limit_buckets.json` but the orchestrator's `@default_quickbeam_extractors` list never invoked the new extractor. Five doc-drift findings (Copilot/CodeRabbit) addressed in the same commit:
  - `lib/mix/tasks/ccxt_extract.update.ex` — added `ccxt_extract.rate_limit_buckets` to `@default_quickbeam_extractors` (P1).
  - `lib/ccxt_extract/rate_limit_buckets.ex` moduledoc — documented the third `unresolved_reason` (`"no_discovery_entry"`) and clarified it is emitted by the Elixir-side pipeline fallback, not the JS extractor.
  - `lib/ccxt_extract/rate_limit_buckets.ex` `empty_record/0` docstring — clarified the wrapper mirrors the JS extractor's *shape*, not its reasons.
  - `lib/ccxt_extract/schema.ex` `build_exchange_v4/4` docstring — flipped `rate_limits` from "out of scope, empty placeholder" to "populated as of Task 89".
  - `lib/ccxt_extract/discovery_loader.ex` moduledoc — dropped the literal "14 discovery files" count (count rotted across parallel Tasks 87 + 89 + 129; counts in moduledocs are a maintenance liability).
  - `lib/mix/tasks/ccxt_extract.rate_limit_buckets.ex` — added `@spec run([String.t()]) :: :ok`.
- **Rebase-into-development merge.** PR #11 opened against `development` before Tasks 87 + 129 merged on `development`; the post-fix-up branch needed a merge from `zenhive/development` to clear conflicts. Conflict surfaces were all additive parallelism: `provenance.ex` `@derived_pointers_v4` (combined `/rate_limits/buckets` from this task with `/rate_limits/per_endpoint_cost` + `/normalization/*` from Tasks 73/129), `schema.ex` `build_exchange_v4/4` body (combined populated-`rate_limits.buckets` with populated-`normalization`), `priv/schema/exchange_v4.json` `RateLimits` properties + new `$defs` (kept all four: `RateLimitBuckets`, `RateLimitBucketEntry`, `RateLimitCosts`, `RateLimitCostEntry`), `pipeline_test.exs` v4 emit assertions (combined buckets-populated + normalization-carrier checks). `.sobelow-skips` regenerated post-merge via `mix sobelow --mark-skip-all`.
- **Files modified.** `lib/ccxt_extract/rate_limit_buckets.ex` (new); `lib/mix/tasks/ccxt_extract.rate_limit_buckets.ex` (new); `lib/ccxt_extract/discovery_loader.ex` (load + key + moduledoc); `lib/ccxt_extract/pipeline.ex` (`get_rate_limit_buckets/2` + parent fallback); `lib/ccxt_extract/schema.ex` (v3 `structure.rate_limit_buckets` + v4 `rate_limits.buckets` emission, `build_exchange_v4/4` docstring); `lib/ccxt_extract/provenance.ex` (`/structure/rate_limit_buckets` + `/rate_limits/buckets` derived); `lib/mix/tasks/ccxt_extract.update.ex` (orchestrator step); `priv/schema/exchange_v3.json` + `priv/schema/exchange_v4.json` (`$defs` + property); `test/ccxt_extract/rate_limit_buckets_test.exs` (new); `test/ccxt_extract/pipeline_test.exs` (v4 emit assertions); `test/integration/cached/schema_v4_emit_cached_test.exs` (cached corpus assertions); `test/support/exchange_fixtures.ex` (`schema_conformant/2` auto-derives the new field).
- **Stale-corpus note.** `priv/output/<id>.json` was last emitted under schema 3.1.0; the new optional field is absent from on-disk fixtures until `mix ccxt_extract.update` regenerates the corpus. `priv/output/` is gitignored derived state — drift here is expected and not a CI gate.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` Task 59 (multi-bucket rate limiter) — bucket axes / refill / max_size are now spec-readable. Task 90 (per-method cost weighting) is the remaining Phase 14 dependency before T59 is fully unblocked.

### Task 72 — Timestamp source/format extraction (`auth.sign_recipe.timestamp`) (PR #12, INE-62)

- **Shipped 2026-05-08.** Phase 11 + 14 endpoint-invocation slot — `auth.sign_recipe.<section>.timestamp` (v4 canonical path) / `structure.sign_recipe[].timestamp` (v3 grace window) populated alongside the existing `nonce` field. Sibling A-1 of the v4 schema-freeze plan; lands in parallel with A-3 (Task 73).
- **`CcxtExtract.SignRecipe.Nonce.timestamp/2`** — thin alias over the existing `derive/2` AST classifier. Today most CCXT priority exchanges derive their wire-level nonce directly from a millisecond / second / microsecond / nanosecond timestamp (`this.nonce()`, `this.milliseconds()`, `Date.now()`), so the two collapse onto identical `{source, format}` records. Surfacing them as distinct schema fields lets v4 consumers read the canonical `auth.sign_recipe.<section>.timestamp` path while v3 keeps `nonce` available for the grace window. The classifier emits the same closed vocabulary defined by `priv/schema/$defs/Nonce` (`source` ∈ `Date.now`/`microseconds`/`milliseconds` family; `format` ∈ `ms`/`s`/`us`/`ns`). If a future exchange ever ships a nonce that genuinely diverges from the timestamp (counter-based, opaque token), the two derivations split in `Nonce` — until then the data is identical and the two fields stay in lockstep on every emitted recipe.
- **Schema** — additive on both surfaces. v3 (`priv/schema/exchange_v3.json`, `priv/schema/sign_recipe_v1.json`): new required `timestamp` key on each `SignRecipeRecord` (oneOf null/`Nonce`); description on the existing `nonce` field updated to flag the grace-window relationship. v4 (`priv/schema/exchange_v4.json`): `auth.sign_recipe.<section>.timestamp` slot populated against the same `Nonce` `$def`. The two `priv/schema/sign_recipe_v1.json` and `exchange_v3.json#/$defs/SignRecipeRecord` shapes stay in lockstep (enforced by `test/ccxt_extract/sign_recipe_test.exs`).
- **Honesty Rule extended.** `SignRecipe.Derive.resolve_unresolved_reason/1` biconditional widened from 6 fields to 7 — `unresolved_reason` is `nil` iff all of `crypto_op`, `canonical_string`, `signature_placement`, `auth_headers`, `nonce`, `timestamp`, and `pre_sign_transforms` are populated. Adding the seventh field tightens the contract: any recipe that produced a populated record under the prior 6-field rule but cannot resolve a `timestamp` flips back to a non-null `unresolved_reason` until `Nonce.timestamp/2` succeeds for that exchange. Full `derive_test.exs` + `nonce_test.exs` regression matrix (binance ms / deribit ms / bitmex ms / kraken ns / okx ms) added.
- **Provenance** — `/structure/sign_recipe/<section>/timestamp` (v3) and `/auth/sign_recipe/<section>/timestamp` (v4) tagged `derived` (computed projection of the `sign()` AST). `provenance_covers_schema` invariant gates this.
- **Honesty-Rule contract test** — `contract_test.ex` invariant `sign_recipe_unresolved_reason_iff_six_fields` extended to require the 7-field biconditional; pre-existing test name kept for stability with a comment noting the field-count migration.
- **Scope kept tight per A-5 coordination.** No `lib/ccxt_extract/pipeline.ex` registry edit, no `lib/ccxt_extract/schema.ex` default bump — the joint v4 schema bump for the 5 sibling Phase 11/14 freeze-list tasks (72, 73, 73d, 89, 90) is owned by A-5 (Task 90). This task touches only the extractor + `priv/schema/*.json` field shapes, matching the issue body's "Files to NOT modify" list.
- **Review fix-up commit (`55ea870`)** — Tier-2 review applied two Copilot nits locally in worktree `~/_DATA/worktrees/ccxt_extract/ine-62/`, pushed directly to the cursor branch under the worktree-as-scope authorization (project CLAUDE.md retired the cloud-agent push-back-only constraint when Linear delegation was scoped down):
  - **Moduledoc field-count drift** in `lib/ccxt_extract/sign_recipe/derive.ex:30` — prose said "all six fields populate" while the code biconditional moved to seven. Fixed to "all seven fields populate."
  - **Trailing whitespace** in `test/ccxt_extract/sign_recipe/nonce_test.exs:395` test name (`"bitmex — timestamp_ms / string (Date.now() global) "`) — trimmed. Mechanically safe; brittle for grep-based test filtering.
- **Files modified.** `lib/ccxt_extract/sign_recipe/nonce.ex` (`timestamp/2` alias + moduledoc); `lib/ccxt_extract/sign_recipe/derive.ex` (7-field biconditional + moduledoc count fix); `lib/ccxt_extract/sign_recipe.ex` (record assembly threads `timestamp`); `lib/ccxt_extract/contract_test.ex` (invariant updated); `priv/schema/sign_recipe_v1.json` (required + property); `priv/schema/exchange_v3.json` (`SignRecipeRecord` mirror); `priv/schema/exchange_v4.json` (`auth.sign_recipe.timestamp`); `test/ccxt_extract/sign_recipe/nonce_test.exs` (regression matrix, new file); `test/ccxt_extract/sign_recipe/derive_test.exs` (7-field cases); `test/ccxt_extract/sign_recipe_test.exs` (lockstep assertions); `test/ccxt_extract/contract_test_test.exs` (invariant test); `test/integration/cached/sign_recipe_cached_test.exs` (cached corpus assertions).
- **Stale-corpus note.** `priv/output/<id>.json` was last emitted under schema 3.1.0; the new optional field is absent from on-disk fixtures until `mix ccxt_extract.update` regenerates the corpus. `priv/output/` is gitignored derived state — drift here is expected and not a CI gate.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` — `auth.sign_recipe.<section>.timestamp` is now spec-readable on the v4 canonical path. No specific consumer task unblocked by this alone (the joint v4 cut waits on A-5 / Task 90); feeds the broader signing-recipe consumer work that lands when the schema freeze completes.

### Task 73 — Per-method rate-limit cost + weight axis (PR #8, INE-63)

- **Shipped 2026-05-08.** Phase 11 + 14 endpoint-invocation slot — per-endpoint rate-limit cost weights derived from the resolved `describe().api`. Additive optional fields, no schema-version bump (v3 stayed at 3.1.0 at PR base; later lifted to 3.2.0 by Task 87 and 3.3.0 by Task 73d via separate PRs).
- **`CcxtExtract.RateLimitCosts`** — QuickBEAM extractor. Walks every `<section>.<verb>.<endpoint>` in resolved `describe().api` and emits `%{cost: number-or-null, axes: object}` records keyed by `<section>.<verb>.<endpoint>`. `cost` is the numeric multiplier applied to the exchange's base `rateLimit`; `axes` carries non-cost annotations (`byLimit`, `cost1s`, etc.) for tier-aware consumers. Extraction-not-interpretation: computed-cost lambdas (paradex's `cost: () => …`) emit `cost: null` verbatim — downstream consumers decide how to resolve runtime-computed costs.
- **`mix ccxt_extract.rate_limit_costs`** — corpus-level mix task with standard `--tier1/--tier2/--tier3/--dex/--all/--exchange` scope flags via `CcxtExtract.TaskScope`. Writes to `priv/discoveries/rate_limit_costs.json` through `AggregateWriter` (scoped runs merge with existing aggregate). Wired into `mix ccxt_extract.update`'s orchestrator; `Pipeline.build_exchange_data/3` reads via `DiscoveryLoader` and assembles per-exchange JSON.
- **Schema** — additive optional fields. v3: `$defs.StructureData.properties.rate_limit_costs`; v4: `$defs.RateLimits.properties.per_endpoint_cost`. Both reference `$defs.RateLimitCosts` (map of endpoint key → `RateLimitCostEntry`) and `$defs.RateLimitCostEntry` (`additionalProperties: false`, `cost` numeric-or-null required, `axes` object required).
- **Provenance** — `/structure/rate_limit_costs` (v3) and `/rate_limits/per_endpoint_cost` (v4) tagged `derived` (computed projection over resolved `describe().api`).
- **Review findings addressed in fix-up commit** (4 push-back findings applied locally as `7604570`, pushed directly to the cursor branch under user-scoped authorization since Cursor Cloud usage was exhausted at review time):
  - **Object-config default cost.** When an endpoint config is `{<axis>: …}` without an own `cost` key, default to `1` per CCXT's `safeValue(options, 'cost', 1)` at `Exchange.ts:1399`. Prior code emitted `cost: null` for this shape.
  - **Array-of-object endpoint shape.** `[{path: '…', cost: N, …}]` now extracts the same `{cost, axes}` shape as the object-keyed shape; previously the array branch only handled string paths.
  - **Test coverage for null-cost path.** Added `synthetic_lambda` fixture in `test/ccxt_extract/rate_limit_costs_test.exs` exercising both null-cost-no-axes and null-cost-with-axes branches through the writer + dispatch counts.
  - **Moduledoc clarification.** Reworded the "what produces null" paragraph to make the paradex-style `cost: () => …` case explicit; removed a stale `__undefined` JSON-sentinel reference.
- **Codex P1 not addressed — intentional.** Codex flagged a concern about implicit-API endpoint inheritance from parent classes; out of scope per the PR's "Files-to-NOT-modify" list and the reviewer note that this concern belongs to a separate task.
- **Files modified.** `lib/ccxt_extract/rate_limit_costs.ex` (new); `lib/mix/tasks/ccxt_extract.rate_limit_costs.ex` (new); `lib/ccxt_extract/discovery_loader.ex` (load + key); `lib/ccxt_extract/pipeline.ex` (wire into `build_exchange_data/3`); `lib/ccxt_extract/provenance.ex` (`/structure/rate_limit_costs` + `/rate_limits/per_endpoint_cost` in derived); `lib/mix/tasks/ccxt_extract.update.ex` (orchestrator step); `priv/schema/exchange_v3.json` + `priv/schema/exchange_v4.json` (`$defs` + property); `test/ccxt_extract/rate_limit_costs_test.exs` (new); `test/support/exchange_fixtures.ex` (`schema_conformant/2` builder).
- **Local-fix-up provenance.** PR #8 was authored by Cursor (`cursor/task-73-rate-limit-costs-b08e`); cloud-agent usage exhausted at review time, so the four push-back findings were applied locally and pushed directly to the cursor branch under user-scoped authorization (a one-time scope-bound override of `delegation-rules.md` § "NEVER PUSH TO A CLOUD-AGENT'S BRANCH"). PR squash-merged as `5b5c904`. See INE-63 close-out comment.
- **Stale-corpus note.** `priv/output/<id>.json` was last emitted under schema 3.1.0; the new optional field is absent from on-disk fixtures until `mix ccxt_extract.update` regenerates the corpus. `priv/output/` is gitignored derived state — drift here is expected and not a CI gate.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` — per-endpoint cost weights are now spec-readable for tier-aware rate-limit consumers. No specific consumer task unblocked by this alone, but feeds the broader rate-limit accounting work in Phase 14 (Tasks 89, 90).

### Task 73d — `transaction_classification` flag (schema 3.3.0, PR #9, INE-64)

- **Shipped 2026-05-08.** Phase 11 request-shape slot lands. Schema bumped to **3.3.0** (additive — new required field `structure.transaction_classification` in v3, mirrored as `endpoints.transaction_classification` in v4). Originally drafted at 3.2.0 by the implementing Cursor agent; bumped to 3.3.0 during local rebase because Task 87 had already taken 3.2.0 on `development` (parallel-PR version collision, resolved by monotonic-version policy).
- **`CcxtExtract.TransactionClassification`** — derives `%{"transactional" => bool, "on_chain" => bool}` per unified-endpoint name from CCXT's naming convention. `transactional` = NOT `fetch*` (everything that places, edits, cancels orders, withdraws, transfers, etc. is a write); `on_chain` = `withdraw*` (strictly narrower; internal `transfer` is exchange-internal and stays off-chain). Pure function from `structure.unified_endpoints` — no new pipeline registry plumbing.
- **Schema 3.3.0** — `structure.transaction_classification` (v3) and `endpoints.transaction_classification` (v4) added as required keys (oneOf null/object). `null` permitted for the no-unified-endpoints case (alias exchanges, empty maps). New `TransactionClassificationEntry` `$def` in both schemas (`additionalProperties: false`, `transactional` + `on_chain` boolean required).
- **Provenance** — both v3 `/structure/transaction_classification` and v4 `/endpoints/transaction_classification` in the `derived` bucket. Tagged `derived` because the booleans are computed projections of unified-method names.
- **Override-registry v3→v4 translation** — `OverrideRegistry.@v3_to_v4_pointer_prefixes` extended with `/structure/transaction_classification` → `/endpoints/transaction_classification`. Same commit also closes a pre-existing gap from Task 87: `/structure/error_class_hierarchy` → `/errors/class_hierarchy` (Task 87 added the field but missed the translation entry — would have silently failed with `KeyError` at `apply_all/2` if a v3-shaped override targeting either path were applied under `--schema-target=4`). Six new tests in `override_registry_test.exs` cover the `translate_pointer/2` contract end-to-end (schema_target == 3 identity, schema_target == 4 prefix rewrites, sub-path preservation, unknown-prefix passthrough). Function had zero direct test coverage before this commit.
- **Security framing — name-only scope, deliberately bounded.** The classifier covers CCXT's unified-method namespace only. Non-unified raw broadcast endpoints — DEX flows that expose blockchain transactions through helpers like `signL1Action` / `signEIP712`, or implicit-API paths like `public_post_sendtx` / `sendTxBatch` — are out of scope: they are not unified-method keys and do not appear in `transaction_classification`. The moduledoc explicitly flags this gap and tells consumers MUST NOT treat `on_chain == false` (or absence) as a sufficient safety gate for blockchain-broadcast operations. Tracked as **Task 73f** in [ROADMAP.md](ROADMAP.md) (OXC body-inspection extension to extend coverage).
- **Files modified.** `lib/ccxt_extract/transaction_classification.ex` (new); `lib/ccxt_extract/schema.ex` (3.2.0 → 3.3.0, required key, v3 + v4 emission); `lib/ccxt_extract/override_registry.ex` (two new translation entries); `lib/ccxt_extract/provenance.ex` (`/structure/transaction_classification` + `/endpoints/transaction_classification` in derived); `priv/schema/exchange_v3.json` (const + required + property + `$def`); `priv/schema/exchange_v4.json` (endpoints required + property + `$def`); `test/ccxt_extract/transaction_classification_test.exs` (new); `test/ccxt_extract/override_registry_test.exs` (`translate_pointer/2` describe block); `test/support/exchange_fixtures.ex` (`schema_conformant/2` auto-derives the new field).
- **Local-fix-up provenance.** The originating PR (#9) was authored by Cursor; out of credit at review time, so a local follow-up branch (`fix/ine-64-rebased`) carried the rebase + bookkeeping + bot-review fixes (CodeRabbit `TODO:` prefix, Copilot v3→v4 translation entry, Codex P1 security-gap moduledoc paragraph). Cursor's commit is preserved in the rebased history; this commit's authorship is the reviewer's. See the Linear issue close-out comment on INE-64.
- **Stale-corpus note.** `priv/output/<id>.json` was last emitted under schema 3.1.0 (drift accumulated through 3.2.0 / Task 87 and now 3.3.0 / Task 73d). The cached `request_shape` schema-version drift detector test fires on this drift as designed; `mix ccxt_extract.update` regenerates the corpus when the user is ready. `priv/output/` is gitignored derived state — drift here is expected and not a CI gate.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` Task 117 (replace hardcoded DEX safelists with spec-driven read) is partially unblocked — the unified-method side is now spec-readable, but the raw-broadcast side requires Task 73f.

### Task 87 — Error class hierarchy export (🎁 13-classify)

- **Shipped 2026-05-08.** Phase 13's first classify-tier slot lands. Closes the third 🎁 **13-classify** task (85, 86 still ⬜). Schema bumped to **3.2.0** (additive — new required field `structure.error_class_hierarchy`); v3 const updated; v4 schema's `errors.class_hierarchy` slot populated alongside the existing `errors.handle_errors`.
- **`CcxtExtract.ErrorClassHierarchy`** — extracts the `BaseError → ExchangeError → ... → AccountNotEnabled` exception taxonomy from `priv/ccxt/ts/src/base/errorHierarchy.ts` (literal nested-object declaration). OXC-parses the source, resolves the `export default <ident>` to its declaring `ObjectExpression`, walks the literal tree into three projections published in lockstep:
  - `tree` — recursive Elixir map mirroring the source literal.
  - `flat_parents` — `class_name => parent_name` (root → `nil`). O(1) parent lookup.
  - `ancestors` — `class_name => [parent, grandparent, ..., BaseError]` pre-computed root chain (empty for the root). O(1) ancestor lookup so consumers don't recurse.
- **Honesty Rule** — no guesses. The source file is a literal declaration; if OXC can't resolve the export's `ObjectExpression` (file moved, refactored to dynamic construction, etc.) `extract/0` returns `{:error, reason}` and the mix task raises. Three explicit error reasons: `:no_default_export`, `:unexpected_default_export_shape`, `{:identifier_not_found, name}`.
- **`mix ccxt_extract.error_class_hierarchy`** — corpus-level mix task (parallel to `mix ccxt_extract.classes`). Single OXC parse, single discovery file. Tier scope is stamped for traceability but does not filter the tree — same rationale as `Classes.write!/2`. Wired into `mix ccxt_extract.update`'s orchestrator; `Pipeline` reads from `priv/discoveries/error_class_hierarchy.json` via `DiscoveryLoader` and assembles into every per-exchange `structure.error_class_hierarchy` (corpus-global data, copied per-exchange so consumers read one canonical shape per file — matches `error_code_fields` / `throw_dispatches` colocation).
- **Schema 3.2.0** — `structure.error_class_hierarchy` added as a required key (oneOf null/object). `null` permitted only for the alias-exchange shape where the entire structure block is null; concrete exchanges always carry the full record. Two new `$defs`: `ErrorClassHierarchyTree` (self-recursive via `additionalProperties: { "$ref": "#/$defs/ErrorClassHierarchyTree" }` — JSV resolves lazily, no infinite expansion at validation time) and `ErrorClassHierarchy` (required: tree/flat_parents/ancestors). v4's `errors.class_hierarchy` peer to `errors.handle_errors` (not nested — class hierarchy is corpus-global; nesting under handle_errors would duplicate 110×).
- **Two contract-test invariants** added to `CcxtExtract.ContractTest`:
  - `error_class_hierarchy_shape_valid` — defends the intrinsic contract: required keys present, tree is a map, exactly one root named `BaseError`, every parent in `flat_parents` is a known class name, every `ancestors[c]` agrees with a `walk_parent_chain` of `flat_parents`. Cycle-safe via a visited-set walk (no infinite recursion if the input is malformed).
  - `error_classes_covered_by_hierarchy` — defends the cross-section contract: every class name referenced in `handle_errors.exceptions[group][message]` and `handle_errors.http_exceptions[status]` exists in `flat_parents`. Catches drift between CCXT's `errorHierarchy.ts` and `BaseError.ts` (a class added to exception maps but missing from the taxonomy export).
- **Provenance** — both `/structure/error_class_hierarchy` (v3) and `/errors/class_hierarchy` (v4) land in the `derived` provenance bucket. Tagged `derived` not `raw` because `flat_parents` and `ancestors` are pre-computed projections — the source has only the nested literal; the two derivations are our work and consumers depend on us computing them right. `provenance_covers_schema` invariant gates this.
- **Tests** — three layers per the project conventions:
  - `test/ccxt_extract/error_class_hierarchy_test.exs` (unit, atom-keyed synthetic OXC ASTs) — happy paths (linear, branching, depth-4 chain, leaves, literal-export shape), error paths (the three named `:error` reasons), `required_keys/0`, `write!/2` envelope, plus an `:extraction`-tagged real-source test asserting the well-known `AccountNotEnabled` depth-4 chain.
  - `test/integration/cached/error_class_hierarchy_cached_test.exs` — reads the committed discovery JSON, asserts envelope structure, root + parent contract, well-known taxonomy fixtures (`AccountNotEnabled`, `RateLimitExceeded → NetworkError`, `AuthenticationError → ExchangeError`).
  - `test/ccxt_extract/contract_test_test.exs` — 14 new tests across the two invariants (valid hierarchy, null hierarchy, missing keys, non-map tree, cycle, wrong root, multiple roots, walk disagreement, exceptions/http_exceptions coverage gaps).
- **Files modified.** `lib/ccxt_extract/error_class_hierarchy.ex` (new); `lib/mix/tasks/ccxt_extract.error_class_hierarchy.ex` (new); `lib/ccxt_extract/discovery_loader.ex` (load + key); `lib/ccxt_extract/pipeline.ex` (wire into `build_exchange_data/3`); `lib/ccxt_extract/schema.ex` (3.1.0 → 3.2.0, required key, v4 errors slot); `lib/ccxt_extract/provenance.ex` (`/structure/error_class_hierarchy` + `/errors/class_hierarchy` in derived); `lib/ccxt_extract/contract_test.ex` (two new invariants + helpers); `lib/mix/tasks/ccxt_extract.update.ex` (orchestrator step); `priv/schema/exchange_v3.json` (const + required + property + two `$defs`); `priv/schema/exchange_v4.json` (errors required + property + two `$defs`); `test/support/exchange_fixtures.ex` (`schema_conformant/2` builder); `test/ccxt_extract/schema_test.exs`, `test/ccxt_extract/validation_test.exs`, `test/ccxt_extract/pipeline_test.exs` (fixtures); plus the three test files listed above.
- **Stale-corpus note.** The on-disk `priv/output/<id>.json` (110 exchanges) was emitted under schema 3.1.0 and will fail `mix ccxt_extract.validate` and the cached integration tests until regenerated via `mix ccxt_extract.update`. This is expected; `priv/output/` is gitignored derived state.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` — error-classification consumer work (Task 58 family) gains a new building block. Phase 13's classify tier still has Tasks 85 (status map) and 86 (retry classification) ⬜; the consumer-side adoption stays blocked until those land. No immediate ccxt_client unblock from this task alone.

### Task 131 — Stabilize 6 seed-dependent flaky tests on `development`

- **Shipped 2026-05-08.** Multi-seed sweep (`mix test.json --quiet --seed 0 / 1 / 42`) all green: 2041 passed, 0 failed, 462 excluded across all three. CI gate at `.github/workflows/harness.yml:89` (`--exclude flaky`) is now operational.
- **Two distinct root causes**, both VM-global state-pollution shapes — the original ROADMAP hypothesis ("PrivWriteCase override leak") was a side-quest, not the load-bearing one.
- **Cluster A — 4 tests in `test/mix/tasks/setup_task_test.exs`** (`check_ts_source/1` cluster). `Setup.check_ts_source/1` calls `Mix.shell().info(...)`; `update_test.exs:460-468`, `contract_test_task_test.exs:14-19`, and `task_helpers.with_captured_shell/1` all swap `Mix.shell` to `Mix.Shell.Process` for message-collection capture. Under `--seed 0` orderings the swap window leaks (or is captured-while-already-polluted), routing output via `:erlang.send` instead of `:stdio` — `capture_io` sees `""` and the `assert output =~ "..."` assertion fails with empty `left`. **Fix:** defensively pin `Mix.shell(Mix.Shell.IO)` in `setup_task_test.exs`'s parent setup, restore `prior_shell` on `on_exit`. Also extended setup to capture/restore `:priv_write_override` (defense-in-depth — `Paths.out_priv_dir/0` reads it before falling through to `:priv_dir_override`, an asymmetry the prior setup didn't cover).
- **Cluster B — 2 tests in `test/ccxt_extract/contract_test_test.exs`** (`check_paths_rw_split/1` cluster). NOT pollution. `Reach.Project.from_glob/1` delegates to `Task.async_stream` with Elixir's hardcoded 5s default timeout; Reach 2.2 doesn't thread a `:timeout` opt through `parse_files`/`build_module_sdgs`. Under the async test pool's CPU contention, even small fixture globs trip the 5s ceiling, and the failure surface widens as the suite warms — quarantining 2 tests exposed a 3rd then 4th in the same file (the `run_all/1` and cross-module-flow tests). **Fix:** `async: false` on the entire file (45 tests serialize, costs ~2s vs flaky CI). Until upstream Reach exposes a timeout knob, this is the durable mitigation.
- **`:flaky` infra** — added `:flaky` to the default `ExUnit.start(exclude: [...])` list in `test/test_helper.exs:50` so local `mix test` agrees with CI's `--exclude flaky` invocation. Permanent infrastructure, not specific to this task. No tests carry the tag in the final state — Phase 1 used it to quarantine while diagnosing, Phase 2 untagged everything.
- **Files modified.** `test/test_helper.exs` (default exclude); `test/mix/tasks/setup_task_test.exs` (parent setup hardened with `Mix.shell` + `:priv_write_override` save/restore); `test/ccxt_extract/contract_test_test.exs` (async: false at file level with explanatory comment about Reach 2.2's timeout default).
- **Follow-up candidate (not filed yet):** upstream Reach issue/PR threading `:timeout` through `parse_files`/`build_module_sdgs` would let the contract-test file return to `async: true`. Score `[D:3/B:3/U:2 → Eff:0.83]` — promote if Reach 3.x doesn't add it organically.

### Tasks 88a/88b/88c — Phase 13 handler routing dispatch tables (PR #6, INE-56)

- **Shipped 2026-05-08** via PR #6 (Cursor-delegated, squash-merged commit `7b5bf2c`). Closes 🎁 **13-dispatch** for all three slots in one bundled PR (file-scope independent — `error_dispatch.ex`, `sign_dispatch.ex`, `parse_dispatch.ex` plus per-module tests). Phases 13-classify (Tasks 85/86/87) remain ⬜.
- **`structure.error_dispatch`** (`CcxtExtract.ErrorDispatch.derive/1`) — flat list of literal `throw new <ExceptionClass>(...)` sites in `handleErrors()`, each entry recording the exception class, the `IfStatement.test` chain that gates it (`predicate_raw`), a coarse classifier (`predicate_kind` ∈ `http_status_in` / `body_contains` / `identifier_check` / `other`), and extracted literal values when classification succeeds.
- **`structure.sign_dispatch`** (`CcxtExtract.SignDispatch.derive/1`) — top-level `if/else if/else` chain in `sign()` enumerated as ordered branch entries; each branch carries the predicate text + normalized `api`/`path` literal matches and the source span of its body. Trailing `is_else: true` entry when `sign()` ends with an `else`. The aggregate `sections` list is the sorted unique union of `api` literals reached across all branches.
- **`structure.parse_dispatch`** (`CcxtExtract.ParseDispatch.derive/1`) — per-method routing table mapping caller method name → sorted unique list of `this.parse<X>(...)` helper names invoked from that method's body. Methods with zero parse calls are omitted. Pipeline reads from `priv/discoveries/parse_methods.json`; alias exchanges fall back to the parent's `parse_dispatch` via `class_hierarchy.json`.
- **Pipeline integration** — `Pipeline.build_exchange_data/3` derives the three sections by passing the assembled `handle_errors`, the effective `sign_method` (own or inherited via `get_parent_sign_method/2`), and the `parse_methods` discovery map; `Schema.build_exchange/4` adds them to the `structure` block. `@required_structure_keys` extended; `exchange_v3.json` updated with the three new `$defs` (`ErrorDispatchEntry`, `SignDispatchData` + `SignDispatchBranch`, `parse_dispatch` inline shape).
- **Bundled revisions** (applied in this commit, not in PR #6) per the `linear-workflow.md` § "Bundled Code-Revisions in Bookkeeping Commit" variant:
  - `lib/ccxt_extract/error_dispatch.ex` — broadened `classify_predicate/1` to recognize bare-identifier truthy checks (`if (code) ...` → `identifier_check`) and `body.indexOf(...) > -1` / `!== -1` (the `> -1` shape uses `UnaryExpression(-, Literal(1))`, not a `Literal -1` — JS AST has no negative literals). Moduledoc updated to reflect actual operator coverage. Copilot + CodeRabbit consensus.
  - `lib/ccxt_extract/error_dispatch.ex` — alternate-branch comment corrected: the predicate chain is **dropped** when descending into `else` branches (the prior comment claimed it was "recorded unchanged" but the code never threaded the test in). Acknowledged limitation; rare in practice. Copilot finding.
  - `lib/ccxt_extract/pipeline.ex:439` — `get_parse_dispatch/2` now falls back to parent when `parse_methods[id]["parse_dispatch"]` is an empty map (previously returned `%{}` and stopped the inheritance walk). Codex P2 finding.
  - `lib/ccxt_extract/schema.ex` — `build_exchange/4` parameter docstring extended to list the three new `structure_data` keys (`error_dispatch`, `sign_dispatch`, `parse_dispatch`). Copilot finding.
  - `priv/schema/exchange_v3.json:455` — `error_dispatch` description split-condition fixed: null = no `handleErrors()` AST reachable; empty list = method walked but contained zero literal `throw new`. Copilot finding.
  - `priv/schema/exchange_v3.json:501` — `predicate_kind` description broadened to mention the full operator set (`===`, `==`, `!==`, `!=`, `>=`, `>`, `<=`, `<`) actually accepted by `classify_predicate/1`, plus the `> -1` / bare-identifier shapes added in this commit. Copilot **CRITICAL** finding (operator coverage drift).
- **Rebase note** — PR #6 was rebased onto `zenhive/development` HEAD (`2c15168`) in a non-mechanical resolution (combined PR #5 envelope-dispatch + PR #7 `Schema.build_exchange_v4/4` additions with PR #6's three new dispatch fields). Force-with-leased to Cursor's branch (`0bead91 → 44d06df`) before merge; Cursor's authorship preserved.
- **`.sobelow-skips` regenerated** — line-fingerprint drift on `pipeline.ex` (5-line addition in `get_parse_dispatch/2` shifted the existing Traversal findings from lines 851/862 to 856/867).
- **Discovered during rebase verification** — Task 131 (now ✅ above) added: 6 seed-dependent flaky tests on `development` (4 in `setup_task_test.exs`, 2 in `contract_test_test.exs`) reproduced identically on upstream HEAD, so they were pre-existing, not introduced by Phase 13. Diagnosis pivot during fix: setup-test cluster was `Mix.shell()` leakage (not priv-dir override leak); contract-test cluster was Reach 2.2's 5s `Task.async_stream` default under pool contention (not pollution at all).
- **Cross-repo:** `../ccxt_client/ROADMAP.md` Task 58 (adopt error contract) — Phase 13 is partial (dispatch ✅, classify ⬜); the consumer adoption task stays ⬜ until Tasks 85/86/87 close. No immediate ccxt_client unblock from this task alone.

### Task 130 — `--schema-target` CLI gate + `Schema.build_exchange_v4/4` (PR #7, INE-60)

- **Shipped 2026-05-08** via PR #7 (Cursor-delegated, squash-merged commit `dabc6e8`). Closes 🎁 **v4-emit** — the v4 schema-freeze gate is now in place. Each freeze-list task can verify its v4 output as it ships; default emission stays v3 until the freeze list empties (atomic cut).
- **`--schema-target=3|4` flag** added to `mix ccxt_extract.pipeline`, `mix ccxt_extract.update`, `mix ccxt_extract.validate` — defaults to `3`, so existing invocations are unchanged. `update` forwards the flag to its sub-stages; `validate` loads the matching schema file from `priv/schema/`.
- **`CcxtExtract.Pipeline.normalize_schema_target!/1`** — public boundary validator; out-of-range values raise `ArgumentError` instead of bubbling up as `FunctionClauseError` from `Schema.schema_filename_for/1` deeper in the call graph.
- **`Schema.build_exchange_v4/4` + `Validation.validate_v4/1` + `Provenance.build_default_v4/0`** added alongside untouched v3 builders. `@schema_version`/`@schema_filename` defaults stay at v3 — flipping the default is a separate post-freeze commit.
- **`priv/schema/exchange_v4.json`** — new gated schema mirroring v3.1.0's field set under consumer-shaped top-level groups (`endpoints`/`auth`/`errors`/`rate_limits`/`normalization`/`markets`/`testnet`/`raw`); `rate_limits` and `normalization` are empty placeholders for future freeze-list tasks (Task 129 carrier + Phase 12 normalization sub-bundles populate them as they ship). Coexists with `exchange_v3.json` per the Task 61c → 107 → 117 grace-window precedent.
- **Override v3→v4 pointer translation** — `OverrideRegistry.translate_pointer/2` rewrites the RFC-6901 paths in committed override files (all currently v3-shaped) to their v4 counterparts (`/structure/authenticated_sections` → `/auth/authenticated_sections`, `/runtime/describe` → `/raw/describe`, etc.) before `put_in/3` lands the value. Threaded through `apply_exchange_overrides/2 → apply_override_entry/5`; provenance stamping uses the translated pointer so override-tagged paths land on the v4 default pointer set. Prefix-based match handles deep pointers like `/structure/handle_errors/exceptions/exact`. Regression test: hyperliquid extracted under `--schema-target=4` validates against `exchange_v4.json` with no rogue top-level `/structure` key.
- **Round-trip validation gated under v4** — `Validation.validate_all/1` treats `schema_target == 4` as implicitly `schema_only: true` with a one-line `Logger.info` notice (round-trip comparators still key on v3 paths; refactor lands with the eventual v4 cut). `mix ccxt_extract.validate --schema-target=4 --exchange binance,deribit,okx` exits clean without requiring `--schema-only`.
- **Tests** — `test/ccxt_extract/pipeline_test.exs` asserts v3 default path is byte-identical for binance/deribit/okx; `test/integration/cached/schema_v4_emit_cached_test.exs` extracts the priority scope under `--schema-target=4`, asserts the consumer-shaped top-level keys present + producer-shaped keys absent, and JSV-validates against `exchange_v4.json`. Cursor's review-round amend tightened the priority-scope assertion from `length == 3` to `MapSet == @priority_scope` exact-set equality.
- **Bot ensemble + reviewer push-back** — CodeRabbit, Copilot, and Codex's GitHub bot all converged on Blocker A (override pointer translation under v4) and Blocker B (round-trip validators key on v3 paths) on the original push. Three reviewer-recipe minors (boundary validation of `:schema_target`, missing `@spec` on duplicated mix-task helpers, exact-set test assertion) folded in across the three amend commits (`d350367`, `97ec152`, `cc6649f`). Final harness green at `cc6649f`; bot ensemble clean.
- **`.sobelow-skips` regenerated** — line-fingerprint drift from the touched `pipeline.ex` / `validation.ex` / `override_registry.ex` files; `mix sobelow --mark-skip-all` applied as part of this bookkeeping commit per the workspace rule.
- **Cross-repo:** `../ccxt_client/ROADMAP.md` Task v4-adopt (🔶 Blocked) stays blocked — the v4 emit gate is now in place but freeze-list tasks (129 + Phase 12 sub-bundles + remaining Phases 11/13/14 endpoint-invocation work) must close before the default flips. No immediate ccxt_client unblock from this task alone.
