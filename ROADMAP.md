...
**Soft gate:** Task 114 (extraction determinism audit) must ship before flipping the v4 emit default — without byte-identical output across runs of the same scope, freeze diffs are noisy and verifying that v4 actually settled is hard.

**v4 emit gate (Task 130):** opt-in `--schema-target=4` flag, mirrors the existing `--pretty` plumbing (CLI flag → opts → `Pipeline.write!/3` → `Schema.build_exchange_v4/4`). v3 stays default until freeze list is empty AND Task 114 is green AND `ccxt_client` has its v4 migration ready. Schema files: `priv/schema/exchange_v3.json` (current) and `priv/schema/exchange_v4.json` (new) coexist during the freeze; v3 retained for one release post-flip per the established Task 61c → Task 107 → Task 117 precedent.
...
|---|--------|-------|-----------|
| 0 | 🎁 **v4-emit** ✅ | Task 130 ✅ | `--schema-target` CLI gate + `Schema.build_exchange_v4/4` — shipped 2026-05-08 via PR #7 (INE-60). Each freeze-list task can now verify its v4 output as it ships. |
| 0 | 🎁 **v4-freeze** | Tasks 70, 71, 72, 73, 73d, 85, 86, 87, 88a, 88b, 88c, 89, 90, 74–83, 129 (NEW) | v4 schema-freeze gate — atomic emission cut. v3 stays the published contract until the freeze list is empty. Soft gate: Task 114 (determinism audit). Bundle extras (in v4 if ready by cut): Tasks 121, 122, 126. |
...
| 6 | 🎁 **11-shape** ✅ | 70, 71 | Verb + path template + body encoding — shipped 2026-05-08 (PR #2, INE-55). |
...
| Task 13b `[CSR]` | ✅ | 🎁 **scope-hygiene** · Universal envelope `tier_scope` stamping — test-migration half [D:2/B:3/U:3 → Eff:1.5] 🚀 — shipped 2026-05-08 via PR #5 (Cursor-delegated, INE-58). 9 cached integration tests now dispatch on the `tier_scope` envelope; `ScopeThresholds` collapses to `full_universe?/1` + `corpus_full_universe?/0` + `proportional/2`. See [CHANGELOG.md](CHANGELOG.md). |
...
| Task 114 `[Opus]` | ⬜ | Audit extraction determinism so the corpus becomes re-committable [D:6/B:6/U:5 → Eff:0.92] 📋 — Untracking (2026-04-18) stopped the bleeding but the root cause is churn, not size: every `mix ccxt_extract.update` produces ~110-file diffs even when upstream CCXT didn't change. Identify and eliminate non-deterministic sources: `generated_at` timestamps, map iteration order through JSON encoding, `AggregateWriter.merge/2` ordering semantics under scoped runs, silent upstream CCXT version drift. Success criterion: same CCXT version + same bundle + same scope → byte-identical output across runs. Once stable, revisit whether to re-track `priv/output/` — size becomes acceptable if diffs are meaningful and infrequent. |
| Task 115 `[CSR]` | ✅ | Self-healing `mix ccxt_extract.setup` — auto-provisions `priv/ccxt`. Shipped 2026-05-08 (PR #4, INE-59). `check_ts_source/1` sparse-clones (`--depth 1 --sparse`, then `git sparse-checkout set ts/src`), pinned to `v<source_version>` from `priv/ccxt_version.json` by default; `--ccxt-version X.Y.Z` overrides, `--latest` opts into the default branch, `--no-clone` flag and `CCXT_EXTRACT_SKIP_CLONE=1` env var preserve the legacy missing-source error for users who symlink their own checkout. README Step 1 collapses to `mix setup`. See [CHANGELOG.md](CHANGELOG.md#task-115--self-healing-mix-ccxt_extractsetup-auto-clones-privccxt-pr-4-ine-59). |
...
| Task 66g | ⬜ | 🎁 **10-sign-extend** · Sub-verb expansion (POST vs PUT vs DELETE vs PATCH) in `canonical_string` [D:3/B:3/U:3 → Eff:1.0] 📋 — `CanonicalString.flip_verb/1` currently maps the else-of-GET branch to `"POST"` for every non-GET verb. Consumers applying the POST recipe to PUT/DELETE/PATCH get the right canonical today because no priority exchange distinguishes those verbs inside sign() — they all fall into the same else-branch. Expand only when a sign() method actually tests `method === 'PUT'` / `=== 'DELETE'` / `=== 'PATCH'` distinctly. Discovered 2026-04-21 during Task 66b. TODO marker at `lib/ccxt_extract/sign_recipe/canonical_string.ex` above the `flip_verb/1` clauses. |
...
| Task 131 `[CSR]` | ✅ | Stabilize 6 seed-dependent flaky tests on `development` [D:3/B:3/U:3 → Eff:1.0] 📋 — fixed 2026-05-08. Two distinct root causes, both VM-global state-pollution shapes (the original "PrivWriteCase override leak" hypothesis was a side-quest, not the load-bearing one). **Cluster A (4 in `setup_task_test.exs`):** `Setup.check_ts_source/1` calls `Mix.shell().info(...)`; `update_test.exs`, `contract_test_task_test.exs`, and `task_helpers.with_captured_shell/1` all swap `Mix.shell` to `Mix.Shell.Process` for message-collection capture. Under specific `--seed 0` orderings the swap leaks (or is captured-while-already-polluted), routing output via `:erlang.send` instead of `:stdio` — `capture_io` sees `""` and assertions fail. **Fix:** defensively pin `Mix.shell(Mix.Shell.IO)` in `setup_task_test.exs`'s parent setup, restore `prior_shell` on `on_exit`. Also extended setup to capture/restore `:priv_write_override` (defense-in-depth — `Paths.out_priv_dir/0` reads it before `:priv_dir_override`). **Cluster B (2 in `contract_test_test.exs`):** NOT pollution — `Reach.Project.from_glob/1` delegates to `Task.async_stream` with Elixir's hardcoded 5s default timeout; Reach 2.2 doesn't thread a `:timeout` opt through `parse_files`/`build_module_sdgs`. Under the async test pool's CPU contention, even small fixture globs trip the 5s ceiling. **Fix:** `async: false` on the file (45 tests serialize, costs ~2s vs flaky CI). Also added `:flaky` to default exclude in `test/test_helper.exs:50` as permanent infra so `--exclude flaky` (in `harness.yml:89`) actually filters when needed. Multi-seed sweep (0/1/42) all green: 2041 passed, 0 failed, 462 excluded. See [CHANGELOG.md](CHANGELOG.md) for details. |
| Task 130 `[CSR]` | ✅ | 🎁 **v4-emit** · `--schema-target` CLI gate + `Schema.build_exchange_v4/4` [D:4/B:8/U:8 → Eff:2.0] 🚀 — shipped 2026-05-08 via PR #7 (INE-60). See [CHANGELOG.md](CHANGELOG.md). **Originally:** Lands before Task 129 so v4 has a target shape to emit into. Mirrors the existing `--pretty` plumbing precedent at `lib/mix/tasks/ccxt_extract.pipeline.ex:56,93` → opts → `Pipeline.write!/3` → `Jason.encode!`. **Steps:** (a) add `--schema-target=3\|4` flag to `mix ccxt_extract.pipeline` and `mix ccxt_extract.update`, default `3`; (b) thread `schema_target:` through `Pipeline.write!/3` opts; (c) add `Schema.build_exchange_v4/4` as the v4 builder, leaving `build_exchange/4` (v3) untouched — branch in `Pipeline.build_exchange_data/3` based on opts; (d) write `priv/schema/exchange_v4.json` (initially the same field set as v3 with the section reorganization shape — `endpoints`/`auth`/`errors`/`rate_limits`/`normalization`/`markets`/`testnet`/`raw`; Phase 12/13/14 tasks populate the new sections as they ship); (e) `mix ccxt_extract.validate --schema-target=4` validates v4 output against `exchange_v4.json`; (f) the existing v3 default path remains until the freeze list is empty AND Task 114 (determinism audit) is green AND `ccxt_client` has its v4 migration ready. **Flipping the default** is a one-line change to `@schema_version "3.1.0"` → `"4.0.0"` and `@schema_filename "exchange_v3.json"` → `"exchange_v4.json"` in `lib/ccxt_extract/schema.ex:57-58`. Old constants retained one release per Task 117 precedent. **🔗 Cross-repo:** flips `../ccxt_client/ROADMAP.md` Task v4-adopt from 🔶 Blocked → ⬜ Pending when v4 becomes default. Discovered 2026-05-08 during v4 schema-freeze drafting. |

### Audit-Surfaced Follow-Ups

> Lower-priority items surfaced by `staged-review:audit-review` passes that don't qualify as same-commit hygiene fixes. Each entry cites the audit SHA + reviewer (Claude or Codex) that surfaced it.

| Task | Status | Notes |
|------|--------|-------|
| Task 132 | ⬜ | 🎁 **docs-drift** · Reconcile `AGENTS.md` cloud-agent guidance with retired `[CSR]`/`[CX]` strategy [D:2/B:3/U:3 → Eff:1.5] 🚀 — `AGENTS.md` (3524 lines, generated from `~/.claude/includes/*.md` per the bundled-include workflow) ships ~15 references to cloud-agent flows (`[CSR]`, `[CX]`, "Cursor Delegation Flow", "Push-Back-vs-Fix-Locally Matrix") that contradict ROADMAP.md § Notes ("cloud-agent delegation retired"). Generator-side fix: either (a) drop the `linear-workflow.md` + `delegation-rules.md` includes from this repo's bundle, (b) regenerate `AGENTS.md` from a slimmer include set, or (c) add a top-of-file callout that cloud-agent sections are reference-only for this repo. Discovered 2026-05-09 by `staged-review:audit-review` on commit 28166a2 (Codex C3). |
| Task 133 | ⬜ | 🎁 **test-coverage** · Add pipeline-level integration tests for `endpoint_cost_binding` propagation [D:3/B:4/U:4 → Eff:1.33] 📋 — Task 90 shipped unit-level coverage on `RateLimitCostBinding.derive/1` (now 7 tests including 3 defensive branches added in audit `28166a2`). Pipeline-level coverage gap: end-to-end propagation through `Pipeline.build_exchange_data/3` → `Schema.build_exchange_v3/v4` → `priv/output/<id>.json` is only validated implicitly via `cached_test` integration suites. Add 3 focused tests against synthetic `rate_limit_buckets` fixtures: (a) wrapper with valid first bucket → both v3 `structure.rate_limits.endpoint_cost_binding` AND v4 `rate_limits.endpoint_cost_binding` populated identically; (b) wrapper with `unresolved_reason != nil` → both paths emit `null`; (c) wrapper missing entirely (no `rate_limit_buckets` slice) → parent fallback emits `null` without raising. Lives in `test/ccxt_extract/pipeline_test.exs` describe-block alongside existing `rate_limit_costs` test scaffolding. Discovered 2026-05-09 by `staged-review:audit-review` on commit 28166a2 (Codex C5 — complementary scope to Claude C1, which was unit-level). |
...
| Task 73e | ⬜ | 🎁 **11+14** · OXC-side extractor for sign-method-constructed `User-Agent` and runtime header mutations [D:5/B:3/U:3 → Eff:0.6] ⚠️ — Captures two QuickBEAM blind spots Task 73b can't reach because the values aren't in resolved `describe()`: (1) `bigone.ts:~1959` constructs `User-Agent` inside `sign()` as `'ccxt/' + this.id + '-' + this.version`; (2) `okx.ts:~7911-7913` mutates `this.headers` at runtime via `setSandboxMode(true)` to inject `x-simulated-trading: 1`. Both need an OXC pass over sign-method bodies to detect: (a) string-concat assignments to a `headers['User-Agent']` LHS, (b) `setSandboxMode`-equivalent runtime mutations on `this.headers`. Output would extend `runtime.request_headers` (or a sibling field) with sign-time / sandbox-time variants. Discovered 2026-05-07 during Task 73b. **Score is low** — affects 2 known exchanges, low consumer impact (most callers don't toggle sandbox via the CCXT-API; they target sandbox URLs directly), high OXC complexity. File but defer until a priority consumer surfaces a concrete need. |
| Task 73f | ⬜ | 🎁 **11-shape** · Extend `transaction_classification` to non-unified raw broadcast endpoints [D:5/B:7/U:7 → Eff:1.4] 📋 — Task 73d shipped a name-only classifier over CCXT's unified-method namespace; consumers correctly cannot trust `on_chain == false` (or absence) as a safety gate for blockchain-broadcast operations because non-unified raw paths bypass the convention. **Coverage gap:** DEX flows that expose blockchain transactions through helpers like `signL1Action` / `signEIP712`, implicit-API paths like `public_post_sendtx` / `sendTxBatch`, and any sign-method body that constructs a signed-payload-then-broadcasts shape. **Approach:** OXC pass over sign-method and exchange-method bodies looking for: (a) calls to known signed-payload helpers (`signL1Action`, `signEIP712`, `signTx`, `signTransaction`); (b) web3 / EIP-712 typed-data builders; (c) `sendTx` / `sendTxBatch` submission patterns; (d) DEX-specific signing imports (`@noble/curves`, `ethers`, `starknet`). Promote endpoints into `transaction_classification` with `on_chain: true` and `transactional: true` (synthesized when no unified entry exists). **Why:** closes the security-gating gap from Task 73d, originally surfaced by the Codex bot review on PR #9. **Consumer uptake:** ccxt_client Task 117 — replaces hardcoded DEX safelists with a spec-driven read of the extended classifier. Filed 2026-05-08 from PR #9 review (INE-64). |
