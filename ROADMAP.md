...
Tasks 65, 64, 61d, 61c, 61a, 37, 60, 58, 57d, 56b, 57b, 59, 57, 56, 55, 54, 53, 52, 49, 47, 46 — see [CHANGELOG.md](CHANGELOG.md).

### 📋 Next Up
| Task | Status | Notes |
|------|--------|-------|
| Task 66a `[P]` | ⬜ | 🎁 **10-HMAC** · Canonical string recipe — HMAC-simple family — next critical-path task [D:5/B:8/U:8 → Eff:1.6] 🚀 |
| Task 66b `[P]` | ⬜ | 🎁 **10-HMAC** · Canonical string recipe — HMAC-with-body family — parallel with 66a [D:5/B:8/U:8 → Eff:1.6] 🚀 |
...
| Task 113 | ⬜ | Git LFS (or trimming) for large generated JSONs [D:3/B:5/U:5 → Eff:1.67] 🚀 — `priv/discoveries/overrides.json` (79MB) is close to GitHub's 100MB hard limit; `priv/discoveries/parse_methods.json` (64MB), `priv/output/binance*.json` (~53MB each), `priv/discoveries/ws_methods.json` (51MB) all exceed the 50MB soft-warning threshold and generate warnings on every push. Options: (a) migrate `priv/output/` and `priv/discoveries/` to Git LFS, (b) compact JSON (no indentation) where round-trip tests allow, (c) split the biggest aggregates by exchange family. Discovered via push warnings after the Task 64 rebase (2026-04-18). |
