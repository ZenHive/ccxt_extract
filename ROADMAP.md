...
| Task 131 `[CSR]` | ⬜ | Stabilize 6 seed-dependent flaky tests on `development` [D:3/B:3/U:3 → Eff:1.0] 📋 — Full-suite run with `--seed 0` deterministically fails 6 tests that pass when the same files run in isolation: (a) `test/mix/tasks/setup_task_test.exs:139,154,183,201` — four `check_ts_source/1` tests (added by Task 115 / PR #4) covering opt-out + auto-clone happy paths; (b) `test/ccxt_extract/contract_test_test.exs` — two `check_paths_rw_split/1` tests (baseline-green and planted-fixture findings, added by Task 112). Repro: `mix test.json --quiet --seed 0` from the repo root fails 6/2503; `mix test test/mix/tasks/setup_task_test.exs` (file alone) passes 17/17. Cause is suite-order state pollution — likely shared `priv` dir / cwd / app env leaking from an earlier `async: false` test that doesn't restore baseline. Both files use `CcxtExtract.PrivWriteCase` (`async: false`); look for an `on_exit/1` callback gap that leaves the override in app env, or a process-dictionary key that survives between tests. Tag offenders with `@tag :flaky` to stabilize CI immediately, then fix the leak and untag. Discovered 2026-05-08 during PR #6 (INE-56) rebase verification — the failures reproduce identically on `zenhive/development` HEAD (`2c15168`), so this is pre-existing, not introduced by Phase 13. |
