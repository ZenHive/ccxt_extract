...
- **ErrorCodeFieldEntry** — `{ object, field, method, field2, roles, sentinel_values }` — a single `this.safe*()` call from handleErrors() with role classification. `object` is the first arg identifier (e.g., `"response"`, `"error"`), `field` is the literal field name accessed, `method` is `"safeString"` / `"safeString2"` / `"safeValue"`, `field2` is the alternate field for safeString2. `roles` is an array of `"error_code"` / `"error_message"` / `"status_sentinel"` derived from AST usage (a field can have multiple roles). `sentinel_values` is the list of literal values from `===`/`!==` comparisons, or null when no sentinel role.
