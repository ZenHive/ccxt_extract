...
**Phase 8 — Client harness + contract tests** complete; Task 57c is the only holdover and is now unblocked after Task 61a provenance shipped (2026-04-17). The target output is a three-tier merge (raw / derived / override). The **generic JSON-Pointer override contract shipped with Task 60** (see CHANGELOG) — override files use RFC 6901 paths, a `value` payload, required `reason`, and `verified_against`/`unverified` flags, validated by `CcxtExtract.OverrideRegistry` and the `override_registry_valid` contract-test invariant. The **generic merge stage shipped with Task 61b** (2026-04-16). **Task 61a shipped 2026-04-17** — every emitted JSON carries a `_provenance` map tagging each section as raw/derived/override; override-applied paths flip to `"override"` at the tail of `Pipeline.extract/1`. **Task 61c shipped 2026-04-17** — `_provenance` is now required and non-null at `schema_version: 2.0.0`; JSON Schema file renamed `exchange_v1.json` → `exchange_v2.json`; SCHEMA.md has migration notes. **Task 61d shipped 2026-04-17** — `provenance_covers_schema` contract-test invariant now fails loudly on drift between `Pipeline`-emitted sections and the `Provenance` declared pointer lists. Phase 9 contract is fully hardened; remaining Phase 9 items (62 validate_overrides, 63 drift_audit, 104 array-index pointers) are additive. **Task 64 shipped 2026-04-18** — Phase 10 opens with the `structure.sign_recipe` scaffold at schema 2.2.0. Every authenticated section gets a per-section recipe record with null derivation fields and `unresolved_reason: "not_yet_derived"`; Tasks 65–69 populate fields incrementally. Two new contract-test invariants (`sign_recipe_keys_match_auth_sections`, `sign_recipe_shape_valid`) lock the shape. **Task 65 shipped 2026-04-18** — `crypto_op` + `signature_placement` now populated via the new `CcxtExtract.SignRecipe.Derive` module. 10 of 12 audited priority exchanges (okx, kucoin, coinbase, deribit, kraken, bitget, gate, phemex plus htx/coinbase partial) emit real values. Binance/bybit honestly emit `ambiguous_ast` — both dispatch on key format at runtime (RSA vs HMAC); a single crypto_op is not truthful. Hyperliquid emits `custom_signing_family` (real signing lives in `signL1Action`, EIP-712/ECDSA, not `sign()`). Zero new contract-test findings. **Task 66a shipped 2026-04-19** — `canonical_string` gets its first derivation pass. Exploration surfaced that priority HMAC exchanges branch canonical strings per HTTP verb (GET = query-only, POST = body-concatenated), so Task 64's single-slot shape couldn't represent them. Schema bumped 2.2.0 → 2.3.0 (additive): `canonical_string` is now a per-verb map keyed on `GET/POST/PUT/DELETE/PATCH` or the sentinel `*`. A new `CcxtExtract.SignRecipe.CanonicalString` module walks `sign()` IfStatement-on-method branches, decomposes `+`-chains into schema-vocabulary components, substitutes local-const identifiers, and drops any branch referencing `body` (that's 66b's scope). First-run coverage is intentionally thin: `okx.private.GET` emits `[timestamp, method, path, literal("?"), query]`. Every other priority exchange stays null with a truthful reason — Binance/Bybit short-circuit on `ambiguous_ast` from Task 65; Hyperliquid on `custom_signing_family`; Kraken/Gate/Deribit/HTX need the expanded source vocabulary from the new Task 66e; KuCoin/Coinbaseexchange need per-verb-reassignment-in-chain analysis (lives in 66b or a 66a iteration). Contract-test invariants stay clean. Next up in 🎁 **10-HMAC** is Task 66b (HMAC-with-body — the POST/PUT slots of the same per-verb map).
...
Tasks 66a, 65, 64, 61d, 61c, 61a, 37, 60, 58, 57d, 56b, 57b, 59, 57, 56, 55, 54, 53, 52, 49, 47, 46 — see [CHANGELOG.md](CHANGELOG.md).
