...
**Parallel packaging-axis work (2026-04-19).** Tasks 116 + 117 run on a separate axis from the endpoint-invocation critical path and **ship immediately** — `ccxt_client` consumer agents have been blocked on the 0.6.0 Hex publish (spec corpus 279MB vs 128MB cap). T116 is a ~10-minute compact-encoding flip (~48% cut); T117 is a mechanical prune of three fields with zero live readers (measured via cross-repo grep of `ccxt_client/{lib,test}/`) behind a schema 3.0.0 bump. Together they take the corpus to ~90MB with ≥30% Hex-cap headroom. Coordination point: `../ccxt_client/ROADMAP.md` Task 105 (SymbolResolver migration + spec_test presence-check update, ~5 LOC) must land in the same release window. Phase 10 work continues in parallel — T66b (HMAC-with-body canonical string) is the next endpoint-invocation critical-path task once 116 + 117 close.
...
| 13 | 🎁 **spec-size** | 116, 117 | Compact encoding (A) + dead-weight prune with schema 3.0.0 breaking change (B) — clears ccxt_client Hex 128MB publish cap. Sequential: measure after A, then ship B. |
...
Tasks 100, 66a, 65, 64, 61d, 61c, 61a, 37, 60, 58, 57d, 56b, 57b, 59, 57, 56, 55, 54, 53, 52, 49, 47, 46 — see [CHANGELOG.md](CHANGELOG.md).
...
| Task 116 🔥 | ⬜ | 🎁 **spec-size · A** · Compact JSON on per-exchange spec writes (`pipeline.ex` + `aggregate_writer.ex`); keep pretty on fixtures + envelope files; add `--pretty` debug flag. Measured ~48% reduction (binance 54MB → 27.8MB). Not sufficient alone to clear Hex 128MB cap on `ccxt_client` — pairs with 117. **Unblocks ccxt_client 0.6.0 Hex publish.** [D:2/B:9/U:9 → Eff:4.5] 🎯 |
| Task 117 🔥 | ⬜ | 🎁 **spec-size · B** · Schema 3.0.0 (breaking) — replace `runtime.markets.markets` with compact `runtime.symbols_index` (23.6MB → ~30KB on binance), drop `structure.parse_methods` (24.5MB corpus-wide) and `structure.ws_methods` (19.8MB corpus-wide). All three fields have zero live readers in `ccxt_client/{lib,test}/` except one presence test covered by ccxt_client T105. Land alongside `../ccxt_client/ROADMAP.md` Task 105. **Unblocks ccxt_client 0.6.0 Hex publish** (with ≥30% Hex-cap headroom). [D:4/B:10/U:10 → Eff:3.0] 🎯 |
| Task 66b `[P]` | ⬜ | 🎁 **10-HMAC** · Canonical string recipe — HMAC-with-body family — populates the `POST`/`PUT` verbs of the 2.3.0 per-verb map next to 66a's `GET` entries [D:5/B:8/U:8 → Eff:1.6] 🚀 |
...
| Task 66b `[P]` | ⬜ | 🎁 **10-HMAC** · Canonical string recipe — HMAC-with-body family (POST/PUT entries of the 2.3.0 per-verb map) [D:5/B:8/U:8 → Eff:1.6] 🚀 |
