defmodule CcxtExtract.Integration.Cached.ExchangesCachedTest do
  @moduledoc """
  Structure tests for exchanges.json — reads cached discovery output.
  Same assertions as ExchangesIntegrationTest but without QuickBEAM boot.
  """
  ...
  describe "structure" do
    test "contains 100+ exchanges with correct field types", %{exchanges: exchanges} do
  ...
  describe "known aliases are correctly marked" do
    for {alias_id, _parent} <- @known_aliases do
      test "#{alias_id} is marked as alias", %{exchanges: exchanges} do
        ex = Enum.find(exchanges, &(&1["id"] == unquote(alias_id)))
