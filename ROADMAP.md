...
| Task | Status | Notes |
|------|--------|-------|
| Task 129 | 🔄 in-review | 🎁 **v4-freeze** · `normalization` block carrier — scaffold + parse_methods digest [D:3/B:6/U:7 → Eff:2.17] 🚀 — PR #10 (INE-61). Lands before Phase 12 Tasks 74–83 (and AFTER Task 130 emit-gate plumbing). New `lib/ccxt_extract/normalization.ex` module wired into `Pipeline.build_exchange_data/3` under v4 schema target only — at v3 emission this is a no-op, preserving the v3 contract through the freeze. **(a) Project `priv/discoveries/parse_methods.json` per-exchange entries into a compact `normalization.parse_methods_digest`** — keys are method names, values are `{params, return_type, async, statement_count}` derived from MethodAST records. **No AST body** — re-emitting full bodies regresses the 91.6% Hex-cap reduction Task 117 shipped. **(b) Scaffold `normalization.field_maps`** with one stub key per parser type (`ticker`/`trade`/`ohlcv`/`order`/`position`/`balance`/`market`/`transaction`/`deposit_address`), each value `null`, top-level `_unresolved_reason: "not_yet_derived"` mirroring sign_recipe scaffold convention (Task 64 precedent). **(c) Scaffold `normalization.response_envelopes`** similarly with `_unresolved_reason: "not_yet_derived"`. Provenance: `/normalization/parse_methods_digest`, `/normalization/field_maps`, `/normalization/response_envelopes` all = `derived`. JSON Schema: define under `priv/schema/exchange_v4.json` `$defs/Normalization`. Two new contract-test invariants: `normalization_shape_valid` (per-exchange shape check, mirrors `testnet_urls_shape_valid` structure) and `parse_methods_digest_covers_inventory` (every method named in `priv/discoveries/parse_methods.json` per-exchange entry surfaces in the digest, modulo override). **🔗 Cross-repo:** on landing, no immediate ccxt_client unblock — Phase 12 tasks 74–83 layer the actual derived maps on top; ccxt_client's v4-adopt task picks up the populated field_maps once Phase 12 closes. Discovered 2026-05-08 during v4 schema-freeze drafting. |
