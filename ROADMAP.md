...
| Task 101 | 🔶 | Migrate to oxc 0.7 + quickbeam 0.10 (atom-keyed AST) [D:4/B:6/U:5 → Eff:1.38] — source migration complete; coincatch orphan cleared by this commit; **still blocked on cached fixture refresh** for `parse_methods` coverage threshold. Run full `mix ccxt_extract.update` to clear. See [CHANGELOG.md](CHANGELOG.md#task-101) and `SCOPED-EXTRACTION-TASKS.md` "Known drift". |
