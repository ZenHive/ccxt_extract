...
Raw extraction runs for every CCXT exchange regardless of tier — and as of **2026-06-14**, so does **derivation**. With `feature_complete` closed (the v4 contract shipped to `../ccxt_client/`), the 7-exchange derivation narrowing is **retired**: derivation (signing recipes, normalization, request/response shapes, error handlers) runs **full-universe by default**, so an unscoped `mix ccxt_extract.update` (`Scope = :all`) populates derived fields for all 110+ exchanges. Derived fields are `null + reason` only when **extraction itself can't prove them** (`ambiguous_ast`, `custom_signing_family`, `no_sign_method`) — never because an exchange is "out of priority scope." **There is no derivation code-gate on tier** (verified 2026-06-14: the only `get_priority_tier/1` consumer is `schema.ex`, which *stamps* the tier label); the prior narrowing was purely operational — the corpus was last regenerated with `--tier1 --dex`, so non-priority exchanges simply never had the derivation pass invoked over them.

The priority tiers in `priv/priority_tiers.json` survive as **priority/ordering labels, not a derivation cap**: they drive `--tierN` dispatch scoping (regenerate one family group in isolation) and the schema `tier` stamp. Variants inherit their root's tier via `class_hierarchy.json`. `tier2` stays as the empty staging bucket for promoting a root's *priority*, but promotion no longer unlocks derivation — everything is already derived. To regenerate the full-universe derived corpus, run `mix ccxt_extract.update` unscoped (Task 152).

**Pre-narrow tier curation (frozen 2026-05-20, historical).** The narrowing this table documents was itself **retired 2026-06-14** (full-universe derivation, above) — kept now only as the reference for *priority* re-curation, not derivation re-scoping. Before the narrowing to the 7-exchange option-seller set, the tiers were:
