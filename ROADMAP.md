...
| Task 67 | ✅ | 🎁 **10-finish** · Auth header set + nonce source derivation SHIPPED 2026-04-24 — new `AuthHeaders` + `Nonce` modules; shared `ASTHelpers` extracted. `auth_headers` populated on 6 priority exchanges (okx, coinbaseexchange, gate, kraken, bitfinex, aster); empty-list on signature-only shapes (deribit, htx × 4 sections); terminal-null on binance / bybit (ambiguous_ast) and hyperliquid (custom_signing_family). Nonce populated on 17 sections total. Kucoin `auth_headers` stays null — `this.extend({...}, headers)` init shape is tracked as an MVP gap (see CHANGELOG). See [CHANGELOG.md](CHANGELOG.md). |
