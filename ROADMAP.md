...
Tasks 61d, 61c, 61a, 37, 60, 58, 57d, 56b, 57b, 59, 57, 56, 55, 54, 53, 52, 49, 47, 46 — see [CHANGELOG.md](CHANGELOG.md).

### 📋 Next Up
| Task | Status | Notes |
|------|--------|-------|
| Task 64 | ⬜ | 🎁 **10-core** · Signing recipe schema design — next critical-path bundle [D:5/B:9/U:9 → Eff:1.8] 🚀 |
...
| Task 107 | ⬜ | Delete `priv/schema/exchange_v1.json` [D:1/B:2/U:2 → Eff:2.0] 📋 — retained one release during the 2.0.0 bump so maintainers can diff the two schemas. Delete in the next schema release. Discovered during Task 61c. |
| Task 108 | ⬜ | Centralize schema-filename literal [D:1/B:3/U:3 → Eff:3.0] 🎯 — `"exchange_v2.json"` is duplicated across 6 non-docstring call sites (`lib/ccxt_extract/pipeline.ex` ×3 at lines 117, 623, 624; `lib/ccxt_extract/validation.ex` ×2 at lines 29, 205; `lib/ccxt_extract/contract_test.ex:304`). Add a single `@schema_filename` / `schema_filename/0` source of truth (likely in `CcxtExtract.Schema` alongside `@schema_version`, or `CcxtExtract.Paths`) so the next schema rename is a one-line change. Discovered during Task 61c code review. |
...
| Task 111 | ⬜ | Fix `Paths.version_file/0` and `Paths.bundle/0` read/write-split drift [D:2/B:4/U:3 → Eff:1.75] 🚀 — Both helpers (`lib/ccxt_extract/paths.ex:92,100`) are built on `Paths.priv/1` (documented read path) but are used exclusively as **write targets** in `mix ccxt_extract.setup` (`File.cp!` destination at `lib/mix/tasks/ccxt_extract.setup.ex:127`; `File.write!` at `setup.ex:287`). This bypasses `:priv_write_override`, violating the invariant in `CLAUDE.md` §Paths: "When adding a new write site, use `Paths.out(...)` not `Paths.priv(...)`". Low practical blast radius (setup is one-shot, not exercised by `PrivWriteCase` tests) but real doc-vs-code drift. Options: (a) add `out_bundle/0` and `out_version_file/0` companions built on `out/1`, leaving the read helpers for consumers that want canonical-only, or (b) flip both helpers onto `out/1` and confirm no caller depends on read-override-without-write-override. Prefer (a) for safety. Discovered via Reach taint analysis (`Paths.bundle` → `File.cp!`, `Paths.version_file` → `File.write!`). |
| Task 112 | ⬜ | `paths_rw_split` Reach-based contract invariant [D:3/B:7/U:5 → Eff:2.0] 🎯 — Depends on Task 111. Add a contract-test invariant (or `mix ccxt_extract.contract_test` probe, or Credo plug-in) that uses `Reach.Project.taint_analysis` to assert no same-function flow from a read-path `CcxtExtract.Paths` helper (`priv`, `priv_dir`, `discoveries`, `ts_src`, and any future read helpers) into a write-path `File` sink (`write`, `write!`, `mkdir_p*`, `cp*`, `rm*`). Must filter same-caller false positives where both a read and a write path converge inside a shared struct (see `validate_fixtures.ex:43` / `fixture_parity.ex:163` convergence). Locks the read/write-split invariant permanently, would catch Task 111-style drift automatically. `reach ~> 1.2` already in `mix.exs` as `only: [:dev, :test]`. Cost mostly in noise-filtering, not in core analysis. Discovered while onboarding Reach. |
