defmodule CcxtExtract.ScopeCleanupTest do
  ...
  describe "prune_out_of_scope/3" do
    ...
    test ":preserve list keeps non-underscore files", %{tmp: tmp} do
      write_files(tmp, ["exchange_v1.json", "kraken.json"])

      assert {:ok, [removed]} =
               ScopeCleanup.prune_out_of_scope(tmp, MapSet.new(), preserve: ["exchange_v1.json"])

      assert Path.basename(removed) == "kraken.json"
      assert File.exists?(Path.join(tmp, "exchange_v1.json"))
    end
